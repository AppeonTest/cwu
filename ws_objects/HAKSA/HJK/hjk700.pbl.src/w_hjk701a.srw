$PBExportHeader$w_hjk701a.srw
$PBExportComments$[청운대]변동사항Trans 생성(한국교육개발원)
forward
global type w_hjk701a from w_condition_window
end type
type dw_main from uo_input_dwc within w_hjk701a
end type
type dw_2 from uo_search_dwc within w_hjk701a
end type
type dw_con from uo_dwfree within w_hjk701a
end type
type uo_1 from uo_imgbtn within w_hjk701a
end type
end forward
...
type ln_templeft from w_condition_window`ln_templeft within w_hjk701a
end type

type ln_tempright from w_condition_window`ln_tempright within w_hjk701a
end type

type ln_temptop from w_condition_window`ln_temptop within w_hjk701a
end type

type ln_tempbuttom from w_condition_window`ln_tempbuttom within w_hjk701a
end type

type ln_tempbutton from w_condition_window`ln_tempbutton within w_hjk701a
end type

type ln_tempstart from w_condition_window`ln_tempstart within w_hjk701a
end type

type uc_retrieve from w_condition_window`uc_retrieve within w_hjk701a
end type

type uc_insert from w_condition_window`uc_insert within w_hjk701a
end type

type uc_delete from w_condition_window`uc_delete within w_hjk701a
end type

type uc_save from w_condition_window`uc_save within w_hjk701a
end type

type uc_excel from w_condition_window`uc_excel within w_hjk701a
end type

type uc_print from w_condition_window`uc_print within w_hjk701a
end type

type st_line1 from w_condition_window`st_line1 within w_hjk701a
end type

type st_line2 from w_condition_window`st_line2 within w_hjk701a
end type

type st_line3 from w_condition_window`st_line3 within w_hjk701a
end type

type uc_excelroad from w_condition_window`uc_excelroad within w_hjk701a
end type

type ln_dwcon from w_condition_window`ln_dwcon within w_hjk701a
end type

type gb_1 from w_condition_window`gb_1 within w_hjk701a
end type

type gb_2 from w_condition_window`gb_2 within w_hjk701a
end type
...
if li_ans = 2 then
	...
	INSERT INTO "HAKSA"."JAEHAK_HAKJUK_TR"  
		...
		IF SQLCA.SQLCODE = 0 THEN
			commit USING SQLCA;
		ELSE
			MESSAGEBOX('확인', "학적자료이관중 처리가 잘못되었습니다." + SQLCA.SQLERRTEXT)
			RETURN
		END IF
