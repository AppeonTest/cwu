$PBExportHeader$w_hjk407p.srw
$PBExportComments$[청운대]제적증명서출력
forward
global type w_hjk407p from w_condition_window
end type
type dw_main from uo_search_dwc within w_hjk407p
end type
type dw_con from uo_dwfree within w_hjk407p
end type
end forward
...
if (Isnull(ls_hakbun) Or ls_hakbun = '') and (Isnull(ls_name) Or ls_name = '') and (Isnull(ls_jumin) Or ls_jumin = '') Then
	...
	elseif li_count >=2 then
	 ...
	 if ls_gubun = '1' Then	// 한글증명서 
			...
			dw_main.dataobject= 'd_hjk407p_1'
    ...
    elseif ls_gubun = '2' Then
			...
			if ls_ename = '' or isnull(ls_ename) or ls_eaddr = '' or isnull(ls_eaddr) then
				...
				dw_main.dataobject= 'd_hjk407p_2'
				dw_main.settransobject(sqlca)
				dw_main.Modify("datawindow.print.preview=yes")
				li_ans = dw_main.retrieve(ls_hakbun, ls_name, ls_jumin)
...
if ls_gubun = '1' Then  // 한글 증명서
	...
	str_balgup.s[2] 	= '13' // 제적증명서
	str_balgup.w[1]	= w_hjk407p
...
type ln_templeft from w_condition_window`ln_templeft within w_hjk407p
end type

type ln_tempright from w_condition_window`ln_tempright within w_hjk407p
end type

type ln_temptop from w_condition_window`ln_temptop within w_hjk407p
end type

type ln_tempbuttom from w_condition_window`ln_tempbuttom within w_hjk407p
end type

type ln_tempbutton from w_condition_window`ln_tempbutton within w_hjk407p
end type

type ln_tempstart from w_condition_window`ln_tempstart within w_hjk407p
end type

type uc_retrieve from w_condition_window`uc_retrieve within w_hjk407p
end type

type uc_insert from w_condition_window`uc_insert within w_hjk407p
end type

type uc_delete from w_condition_window`uc_delete within w_hjk407p
end type

type uc_save from w_condition_window`uc_save within w_hjk407p
end type

type uc_excel from w_condition_window`uc_excel within w_hjk407p
end type

type uc_print from w_condition_window`uc_print within w_hjk407p
end type

type st_line1 from w_condition_window`st_line1 within w_hjk407p
end type

type st_line2 from w_condition_window`st_line2 within w_hjk407p
end type

type st_line3 from w_condition_window`st_line3 within w_hjk407p
end type

type uc_excelroad from w_condition_window`uc_excelroad within w_hjk407p
end type

type ln_dwcon from w_condition_window`ln_dwcon within w_hjk407p
end type

type gb_1 from w_condition_window`gb_1 within w_hjk407p
end type

type gb_2 from w_condition_window`gb_2 within w_hjk407p
end type
