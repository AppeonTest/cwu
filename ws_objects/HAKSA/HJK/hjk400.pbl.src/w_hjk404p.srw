$PBExportHeader$w_hjk404p.srw
$PBExportComments$[청운대]수료증명서출력
forward
global type w_hjk404p from w_condition_window
end type
type dw_main from uo_search_dwc within w_hjk404p
end type
type dw_con from uo_dwfree within w_hjk404p
end type
end forward
...
integer 	li_ans, li_count, ll_hakjum,	ll_injung_hakjum, ll_tot
...
// 수료증명서 조회시 학생의 34이하이면 수료증명서가 발급 불가능하게 처리
		...
		ll_tot	=	ll_hakjum + ll_injung_hakjum
		
		if ll_tot <= 34 then
			messagebox("확인", ls_hakbun + "의 학생은 " + string(ll_tot) + "학점취득으로 발급 불가능합니다!", Exclamation!)
			dw_con.setfocus()
	         dw_con.SetColumn("hakbun")
			return 1
	   end if
 	...
 	if ls_gubun = '1' then	// 한글증명서
		...
		dw_main.dataobject= 'd_hjk404p_1'
 	...
 	elseif ls_gubun = '2' then
		...
		if ls_ename = '' or isnull(ls_ename) or ls_eaddr = '' or isnull(ls_eaddr) then
			...
			dw_main.dataobject= 'd_hjk404p_2'
			dw_main.settransobject(sqlca)
			dw_main.Modify("datawindow.print.preview=yes")
			li_ans = dw_main.retrieve(ls_hakbun, ls_name, ls_jumin)
...
if ls_gubun = '1' Then  // 한글 증명서
	...
	str_balgup.s[2] 	= '07' // 수료증명서
	str_balgup.w[1]	= w_hjk404p
...
type ln_templeft from w_condition_window`ln_templeft within w_hjk404p
end type

type ln_tempright from w_condition_window`ln_tempright within w_hjk404p
end type

type ln_temptop from w_condition_window`ln_temptop within w_hjk404p
end type

type ln_tempbuttom from w_condition_window`ln_tempbuttom within w_hjk404p
end type

type ln_tempbutton from w_condition_window`ln_tempbutton within w_hjk404p
end type

type ln_tempstart from w_condition_window`ln_tempstart within w_hjk404p
end type

type uc_retrieve from w_condition_window`uc_retrieve within w_hjk404p
end type

type uc_insert from w_condition_window`uc_insert within w_hjk404p
end type

type uc_delete from w_condition_window`uc_delete within w_hjk404p
end type

type uc_save from w_condition_window`uc_save within w_hjk404p
end type

type uc_excel from w_condition_window`uc_excel within w_hjk404p
end type

type uc_print from w_condition_window`uc_print within w_hjk404p
end type

type st_line1 from w_condition_window`st_line1 within w_hjk404p
end type

type st_line2 from w_condition_window`st_line2 within w_hjk404p
end type

type st_line3 from w_condition_window`st_line3 within w_hjk404p
end type

type uc_excelroad from w_condition_window`uc_excelroad within w_hjk404p
end type

type ln_dwcon from w_condition_window`ln_dwcon within w_hjk404p
end type

type gb_1 from w_condition_window`gb_1 within w_hjk404p
end type

type gb_2 from w_condition_window`gb_2 within w_hjk404p
end type
