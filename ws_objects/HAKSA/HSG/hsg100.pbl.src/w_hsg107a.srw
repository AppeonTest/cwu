$PBExportHeader$w_hsg107a.srw
$PBExportComments$[w_list_master_detail] 상담 및 검사 접수(인력개발센타)
forward
global type w_hsg107a from w_window
end type
type dw_list from uo_grid within w_hsg107a
end type
type dw_con from uo_dw within w_hsg107a
end type
type dw_main from uo_dw within w_hsg107a
end type
type uc_row_insert from u_picture within w_hsg107a
end type
type uc_row_delete from u_picture within w_hsg107a
end type
type dw_sub from uo_grid within w_hsg107a
end type
type st_main from statictext within w_hsg107a
end type
type st_detail from statictext within w_hsg107a
end type
type p_2 from picture within w_hsg107a
end type
type p_1 from picture within w_hsg107a
end type
type p_3 from picture within w_hsg107a
end type
type st_sub from statictext within w_hsg107a
end type
type uo_stu from uo_imgbtn within w_hsg107a
end type
type uo_ok from uo_imgbtn within w_hsg107a
end type
type uo_sms from uo_imgbtn within w_hsg107a
end type
type dw_print from datawindow within w_hsg107a
end type
end forward
...
this.Control[iCurrent+13]=this.uo_stu
this.Control[iCurrent+14]=this.uo_ok
this.Control[iCurrent+15]=this.uo_sms
this.Control[iCurrent+16]=this.dw_print
end on

on w_hsg107a.destroy
call super::destroy
...
lvc_data.setProperty('column2', 'case_tp')  //찾은목적
...
//dw_main.object.counseller.tag = 'NOTNULL(상담자)'
...
uo_ok.x = ln_tempright.beginx - uo_ok.width
end event

event ue_savestart;call super::ue_savestart;If dw_main.rowcount() = 0 Then return 1

String	ls_yn, ls_counsel_cd, ls_year, ls_hakgi, ls_hakbun, ls_case_no, ls_cname, ls_act_dt
...
//ls_act_dt = dw_main.object.act_dt[1]
//
//If isnull(ld_act_dt ) or String(ld_act_dt, 'yyyymmdd') = '00000000' Then 
//	dw_main.object.act_dt[1] = func.of_get_datetime()
//End If
...
type ln_templeft from w_window`ln_templeft within w_hsg107a
end type

type ln_tempright from w_window`ln_tempright within w_hsg107a
end type

type ln_temptop from w_window`ln_temptop within w_hsg107a
end type

type ln_tempbuttom from w_window`ln_tempbuttom within w_hsg107a
end type

type ln_tempbutton from w_window`ln_tempbutton within w_hsg107a
end type

type ln_tempstart from w_window`ln_tempstart within w_hsg107a
end type

type uc_retrieve from w_window`uc_retrieve within w_hsg107a
end type

type uc_insert from w_window`uc_insert within w_hsg107a
end type

type uc_delete from w_window`uc_delete within w_hsg107a
end type

type uc_save from w_window`uc_save within w_hsg107a
end type

type uc_excel from w_window`uc_excel within w_hsg107a
end type

type uc_print from w_window`uc_print within w_hsg107a
end type

type st_line1 from w_window`st_line1 within w_hsg107a
end type

type st_line2 from w_window`st_line2 within w_hsg107a
end type

type st_line3 from w_window`st_line3 within w_hsg107a
end type

type uc_excelroad from w_window`uc_excelroad within w_hsg107a
end type
...
String		ls_gwa, ls_gwa_name, ls_member_no, ls_prof_name, ls_purpose
Long		ll_rv
Long 		l_case_no
...
ls_purpose = dw_con.object.case_tp[dw_con.Getrow()]
...
This.object.purpose[This.Getrow()] = ls_purpose
...
This.object.step[This.getrow()] = '4'  //접수
...
If ls_purpose <> '' AND isnull(ls_purpose) = false Then
	this.event itemchanged(1, This.object.purpose, ls_purpose)
Else
	dw_sub.event ue_retrieve()
End If
...
//			this.object.case_tp[row] = '1' //reset
//			
//		Else //검사 선택시
			SELECT CODE, FNAME
...
event constructor;call super::constructor;//this.object.purpose.protect = 1
this.object.tel.protect = 1
this.object.hp.protect = 1
this.object.email.protect = 1
this.object.case_tp.protect = 1
end event
...
dw_main.object.act_dt[ll_row]  = func.of_get_sdate('yyyymmdd')
dw_main.object.step[ll_row] = '4' //접수
dw_main.object.act_mem_no[ll_row] = gs_empcode
...
ls_hp = func.of_replace(trim(dw_main.object.hp[ll_row]), '-', '')
