$PBExportHeader$w_hsg117a.srw
$PBExportComments$[w_list_master] 집단프로그램개설신청승인
forward
global type w_hsg117a from w_window
end type
type dw_list from uo_grid within w_hsg117a
end type
type dw_con from uo_dw within w_hsg117a
end type
type dw_main from uo_dw within w_hsg117a
end type
type p_1 from picture within w_hsg117a
end type
type st_main from statictext within w_hsg117a
end type
type p_2 from picture within w_hsg117a
end type
type st_detail from statictext within w_hsg117a
end type
type uo_ok from uo_imgbtn within w_hsg117a
end type
end forward
...
event ue_button_set;call super::ue_button_set;uo_ok.x = ln_tempright.beginx - uo_ok.width
end event

type ln_templeft from w_window`ln_templeft within w_hsg117a
end type

type ln_tempright from w_window`ln_tempright within w_hsg117a
end type

type ln_temptop from w_window`ln_temptop within w_hsg117a
end type

type ln_tempbuttom from w_window`ln_tempbuttom within w_hsg117a
end type

type ln_tempbutton from w_window`ln_tempbutton within w_hsg117a
end type

type ln_tempstart from w_window`ln_tempstart within w_hsg117a
end type

type uc_retrieve from w_window`uc_retrieve within w_hsg117a
end type

type uc_insert from w_window`uc_insert within w_hsg117a
end type

type uc_delete from w_window`uc_delete within w_hsg117a
end type

type uc_save from w_window`uc_save within w_hsg117a
end type

type uc_excel from w_window`uc_excel within w_hsg117a
end type

type uc_print from w_window`uc_print within w_hsg117a
end type

type st_line1 from w_window`st_line1 within w_hsg117a
end type

type st_line2 from w_window`st_line2 within w_hsg117a
end type

type st_line3 from w_window`st_line3 within w_hsg117a
end type

type uc_excelroad from w_window`uc_excelroad within w_hsg117a
end type
...
String		ls_year, ls_hakgi, ls_member_no, ls_ent_no, ls_tp
Long		ll_rv
...
Else
	ls_tp = func.of_nvl(This.object.act_tp[This.getrow()], '')
	
	If ls_tp  <> '3' Then  //승인이 아닐 경우
		uo_ok.of_enable(true)
	End If
End If
RETURN ll_rv
...
ls_member_no = dw_con.object.member_no[ll_row]
...
//	This.object.kname[This.getrow()] = ls_hname
//End If
...
dw_main.object.ACT_TP[ll_row] = '3' //승인
