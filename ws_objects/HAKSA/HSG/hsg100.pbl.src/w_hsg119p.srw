$PBExportHeader$w_hsg119p.srw
$PBExportComments$[w_list] 소집단 대화활동 실시 현황(통계)
forward
global type w_hsg119p from w_window
end type
type dw_con from uo_dwfree within w_hsg119p
end type
type dw_main from datawindow within w_hsg119p
end type
end forward
...
type ln_templeft from w_window`ln_templeft within w_hsg119p
end type

type ln_tempright from w_window`ln_tempright within w_hsg119p
end type

type ln_temptop from w_window`ln_temptop within w_hsg119p
end type

type ln_tempbuttom from w_window`ln_tempbuttom within w_hsg119p
end type

type ln_tempbutton from w_window`ln_tempbutton within w_hsg119p
end type

type ln_tempstart from w_window`ln_tempstart within w_hsg119p
end type

type uc_retrieve from w_window`uc_retrieve within w_hsg119p
end type

type uc_insert from w_window`uc_insert within w_hsg119p
end type

type uc_delete from w_window`uc_delete within w_hsg119p
end type

type uc_save from w_window`uc_save within w_hsg119p
end type

type uc_excel from w_window`uc_excel within w_hsg119p
end type

type uc_print from w_window`uc_print within w_hsg119p
end type

type st_line1 from w_window`st_line1 within w_hsg119p
end type

type st_line2 from w_window`st_line2 within w_hsg119p
end type

type st_line3 from w_window`st_line3 within w_hsg119p
end type

type uc_excelroad from w_window`uc_excelroad within w_hsg119p
end type
...
string dataobject = "d_hsg119p_con"
...
event type long ue_retrieve();String		ls_year, ls_hakgi, ls_fr_dt, ls_to_dt
Long		ll_rv
...
ll_rv = THIS.Retrieve(ls_year, ls_hakgi, ls_fr_dt, ls_to_dt)
