$PBExportHeader$w_hsg_hakjuk.srw
$PBExportComments$학생 조회
forward
global type w_hsg_hakjuk from window
end type
type sle_cust_name from singlelineedit within w_hsg_hakjuk
end type
type st_1 from statictext within w_hsg_hakjuk
end type
type cb_retrieve from commandbutton within w_hsg_hakjuk
end type
type cb_cancel from commandbutton within w_hsg_hakjuk
end type
type cb_ok from commandbutton within w_hsg_hakjuk
end type
type dw_main from cuo_dwwindow_one_hin within w_hsg_hakjuk
end type
type gb_1 from groupbox within w_hsg_hakjuk
end type
end forward
...
global w_hsg_hakjuk w_hsg_hakjuk
...
lstr_com.ls_item[3] = trim(dw_main.getitemstring(ll_getrow, 'gwa'))
lstr_com.ls_item[4] = trim(dw_main.getitemstring(ll_getrow, 'gwa_name'))
lstr_com.ls_item[5] = trim(dw_main.getitemstring(ll_getrow, 'dr_hakyun'))
...
integer height = 1536
integer taborder = 20
string dataobject = "d_hsg_hakjuk"
