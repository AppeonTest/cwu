$PBExportHeader$w_hac701a.srw
$PBExportComments$추경예산 관리/출력(예산부서용)
forward
global type w_hac701a from w_tabsheet
end type
...
type cb_cal from cuo_cal within w_hac701a
end type
end forward
...
public function integer wf_create_remark (string as_bdgt_year, string as_gwa, string as_acct_code, integer ai_acct_class, string as_io_gubun)
public subroutine wf_retrieve_preview ()
public subroutine wf_delete ()
public subroutine wf_getchild ()
public subroutine wf_insert ()
public subroutine wf_bdgt_check (long al_row, string as_acct_code)
...
idw_data.retrieve(ls_bdgt_year, ls_gwa, ls_acct_code, gi_acct_class, ls_io_gubun, li_bdgt_class, ii_stat_class, is_mast_work_gbn)
...
if f_chkterm(ls_bdgt_year, li_bdgt_class, ii_stat) < 0 then
	ii_stat_class = -1
else
...
if idw_mast.retrieve(ls_bdgt_year, ls_gwa, gi_acct_class, ls_slip_class, li_bdgt_class, ii_stat_class) > 0 then
	wf_retrieve_preview()
else
	idw_preview.reset()
	idw_print.reset()
...
// 작 성 인 : 	이현수
// 작성일자 : 	2002.10
// 함수원형 : 	wf_create_remark(string as_bdgt_year, string	as_gwa, string as_acct_code, integer ai_acct_class, string as_io_gubun)
// 인    수 :	as_bdgt_year	-	bdgt_year
//					as_gwa			-	gwa
//					as_acct_code	-	acct code
//					ai_acct_class	-	acct_class
//					as_io_gubun		-	io_gubun
// 되 돌 림 :	sqlca.sqlcode(integer)
// 주의사항 :
// 수정사항 :
// ==========================================================================================
integer	li_bdgt_seq, li_grp, li_sort
string	ls_calc_remark, ls_remark
long		ll_newrow
Int 	li_bdgt_class
...
declare	cur_1	cursor	for
	select	bdgt_seq, grp, sort, calc_remark, remark
	from		acdb.hac012h
...
fetch		cur_1
	into	:li_bdgt_seq, :li_grp, :li_sort, :ls_calc_remark, :ls_remark	;

idw_data.reset()

do while	sqlca.sqlcode	=	0	
	ll_newrow	=	idw_data.insertrow(0)
	idw_data.setitem(ll_newrow,	'bdgt_year',	as_bdgt_year)
	idw_data.setitem(ll_newrow,	'gwa',			as_gwa)
	idw_data.setitem(ll_newrow,	'acct_code',	as_acct_code)
	idw_data.setitem(ll_newrow,	'acct_class',	ai_acct_class)
	idw_data.setitem(ll_newrow,	'io_gubun',		as_io_gubun)
	idw_data.setitem(ll_newrow,	'bdgt_class',	li_bdgt_class)
	idw_data.setitem(ll_newrow,	'bdgt_seq',		li_bdgt_seq)
	idw_data.setitem(ll_newrow,	'grp',			li_grp)
	idw_data.setitem(ll_newrow,	'sort',			li_sort)
	idw_data.setitem(ll_newrow,	'calc_remark',	ls_calc_remark)
	idw_data.setitem(ll_newrow,	'remark',		ls_remark)
	idw_data.setitem(ll_newrow,	'work_gbn',		'C')
	idw_data.setitem(ll_newrow,	'worker',		gs_empcode)
	idw_data.setitem(ll_newrow,	'ipaddr',		gs_ip)
	idw_data.setitem(ll_newrow,	'work_date',	f_sysdate())
	idw_data.setitem(ll_newrow,	'job_uid',		gs_empcode)
	idw_data.setitem(ll_newrow,	'job_add',		gs_ip)
	idw_data.setitem(ll_newrow,	'job_date',		f_sysdate())
	idw_data.setitem(ll_newrow,	'stat_class',	ii_stat_class)
...
idw_preview.retrieve(ls_bdgt_year, ls_gwa, gi_acct_class, ls_slip_class, li_bdgt_class)

if idw_print.retrieve(ls_bdgt_year, ls_gwa, gi_acct_class, ls_slip_class, li_bdgt_class) < 1 then
...
if isnull(ls_slip_class) or trim(ls_slip_class) = '' then
	f_messagebox('1', '수입지출구분을 정확히 입력하시기 바랍니다.')
	dw_con.setfocus()
	dw_con.setcolumn('slip_class')
	return
...
if idw_mast.rowcount() < 1 then
	// 초기화 하기 위해 강제 조회
	idw_mast.retrieve('ZZZZ', ls_gwa, gi_acct_class, ls_slip_class, li_bdgt_class, ii_stat_class)
end if
...
idw_mast.setitem(li_newrow, 'bdgt_class',	li_bdgt_class)
...
public subroutine wf_bdgt_check (long al_row, string as_acct_code);// ==========================================================================================
// 기    능 : 	본예산금액과 산출근거를 체크한다.
// 작 성 인 : 	이현수
// 작성일자 : 	2002.10
// 함수원형 : 	wf_bdgt_check(long al_row, string as_gwa, string as_acct_code)
// 인    수 :	al_row				-	Row
//					as_acct_code		-	계정과목코드
// 되 돌 림 :	integer
// 주의사항 :
...
ls_bdgt_year  = idw_mast.getitemstring(al_row, 'bdgt_year')
ls_gwa	     = idw_mast.getitemstring(al_row, 'gwa')
li_acct_class = idw_mast.getitemnumber(al_row, 'acct_class')
ls_io_gubun   = idw_mast.getitemstring(al_row, 'io_gubun')

// 본예산금액을 가져온다.
select	nvl(sum(nvl(confirm_amt, 0)), 0)
into		:ldb_bdgt_amt
from		acdb.hac011h
where		bdgt_year			=	:ls_bdgt_year
and		gwa					=	:ls_gwa
...
if sqlca.sqlcode <> 0 then	ldb_bdgt_amt = 0

idw_mast.setitem(al_row, 'bdgt_amt', ldb_bdgt_amt)

// 본예산의 산출근거를 입력한다.
li_count	=	idw_data.rowcount()

if li_count > 0	then
	if f_messagebox('2', '산출근거가 이미 존재합니다.~n~n삭제 후 본예산의 산출근거를 재반영하시겠습니까?')	=	1	then

		// 이미 존재하는 자료를 삭제한다.
		for li_row = li_count to 1	step -1
			idw_data.deleterow(li_row)
		next

		if wf_create_remark(ls_bdgt_year, ls_gwa, as_acct_code, li_acct_class, ls_io_gubun) <> 0 then
...
this.Control[iCurrent+1]=this.cb_cal
end on

on w_hac701a.destroy
call super::destroy
destroy(this.cb_cal)
end on
...
type ln_templeft from w_tabsheet`ln_templeft within w_hac701a
end type

type ln_tempright from w_tabsheet`ln_tempright within w_hac701a
end type

type ln_temptop from w_tabsheet`ln_temptop within w_hac701a
end type

type ln_tempbuttom from w_tabsheet`ln_tempbuttom within w_hac701a
end type

type ln_tempbutton from w_tabsheet`ln_tempbutton within w_hac701a
end type

type ln_tempstart from w_tabsheet`ln_tempstart within w_hac701a
end type

type uc_retrieve from w_tabsheet`uc_retrieve within w_hac701a
end type

type uc_insert from w_tabsheet`uc_insert within w_hac701a
end type

type uc_delete from w_tabsheet`uc_delete within w_hac701a
end type

type uc_save from w_tabsheet`uc_save within w_hac701a
end type

type uc_excel from w_tabsheet`uc_excel within w_hac701a
end type

type uc_print from w_tabsheet`uc_print within w_hac701a
end type

type st_line1 from w_tabsheet`st_line1 within w_hac701a
...
type st_line2 from w_tabsheet`st_line2 within w_hac701a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type st_line3 from w_tabsheet`st_line3 within w_hac701a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type uc_excelroad from w_tabsheet`uc_excelroad within w_hac701a
end type

type ln_dwcon from w_tabsheet`ln_dwcon within w_hac701a
end type

type tab_sheet from w_tabsheet`tab_sheet within w_hac701a
integer y = 320
integer width = 4379
integer height = 1960
integer textsize = -9
...
if	dwo.name = 'acct_code' then
	...
	if ll_cnt = 1 then
		...
		wf_bdgt_check(row, lstr_Rtn.ls_item[1])
		return 1
	end if
end if
...
type uo_tab from w_tabsheet`uo_tab within w_hac701a
integer x = 1723
long backcolor = 1073741824
end type

type dw_con from w_tabsheet`dw_con within w_hac701a
string dataobject = "d_hac701a_con"
end type

type st_con from w_tabsheet`st_con within w_hac701a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
