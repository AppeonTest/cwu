$PBExportHeader$w_hac302a.srw
$PBExportComments$예산 요구 관리/출력(주관부서용)
forward
global type w_hac302a from w_tabsheet
end type
...
type cb_1 from commandbutton within w_hac302a
end type
type cb_cal from cuo_cal within w_hac302a
end type
type uo_acct_class from cuo_acct_class within w_hac302a
end type
end forward

global type w_hac302a from w_tabsheet
string title = "예산 요구 관리/출력(주관부서용)"
cb_1 cb_1
cb_cal cb_cal
uo_acct_class uo_acct_class
end type
global w_hac302a w_hac302a
...
//
//if gi_dept_opt = 3 then
//	dw_con.object.code.protect = 1//dw_gwa.enabled = False
...
type ln_templeft from w_tabsheet`ln_templeft within w_hac302a
end type

type ln_tempright from w_tabsheet`ln_tempright within w_hac302a
end type

type ln_temptop from w_tabsheet`ln_temptop within w_hac302a
end type

type ln_tempbuttom from w_tabsheet`ln_tempbuttom within w_hac302a
end type

type ln_tempbutton from w_tabsheet`ln_tempbutton within w_hac302a
end type

type ln_tempstart from w_tabsheet`ln_tempstart within w_hac302a
end type

type uc_retrieve from w_tabsheet`uc_retrieve within w_hac302a
end type

type uc_insert from w_tabsheet`uc_insert within w_hac302a
end type

type uc_delete from w_tabsheet`uc_delete within w_hac302a
end type

type uc_save from w_tabsheet`uc_save within w_hac302a
end type

type uc_excel from w_tabsheet`uc_excel within w_hac302a
end type

type uc_print from w_tabsheet`uc_print within w_hac302a
end type

type st_line1 from w_tabsheet`st_line1 within w_hac302a
...
type st_line2 from w_tabsheet`st_line2 within w_hac302a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type st_line3 from w_tabsheet`st_line3 within w_hac302a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type uc_excelroad from w_tabsheet`uc_excelroad within w_hac302a
end type

type ln_dwcon from w_tabsheet`ln_dwcon within w_hac302a
end type

type tab_sheet from w_tabsheet`tab_sheet within w_hac302a
integer x = 55
integer y = 308
integer height = 1976
integer textsize = -9
...
if isnull(getitemstring(currentrow, 'hac011h_work_gbn'))then
	is_mast_work_gbn  =	''
else
	is_mast_work_gbn	=	'C'
end if
...
//selectrow(0, false)
if ii_mast_stat_class <> ii_stat_class or not isnull(getitemstring(currentrow, 'hac011h_work_gbn'))then
...
if isnull(getitemstring(newrow, 'hac011h_work_gbn'))then
	is_mast_work_gbn  =	''
else
	is_mast_work_gbn	=	'C'
end if
...
//selectrow(0, false)
if ii_mast_stat_class <> ii_stat_class or not isnull(getitemstring(newrow, 'hac011h_work_gbn'))then
...
type uo_tab from w_tabsheet`uo_tab within w_hac302a
integer x = 1632
integer y = 448
long backcolor = 1073741824
end type

type dw_con from w_tabsheet`dw_con within w_hac302a
...
type st_con from w_tabsheet`st_con within w_hac302a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
