$PBExportHeader$w_hac201a.srw
$PBExportComments$예산 요구 등록/출력(단위부서용)
forward
global type w_hac201a from w_tabsheet
end type
...
type dw_print1 from datawindow within tabpage_sheet04
end type
type tabpage_sheet04 from userobject within tab_sheet
dw_print1 dw_print1
end type
type cb_1 from commandbutton within w_hac201a
end type
type cb_cal from cuo_cal within w_hac201a
end type
type uo_acct_class from cuo_acct_class within w_hac201a
end type
end forward

global type w_hac201a from w_tabsheet
string title = "예산 요구 등록/출력(단위부서용)"
cb_1 cb_1
cb_cal cb_cal
uo_acct_class uo_acct_class
end type
global w_hac201a w_hac201a

type variables
datawindowchild	idw_child, idw_child2, idw_child_acct_code
datawindow			idw_mast, idw_data, idw_preview,  idw_print1

datawindow			idw_1

string				is_bdgt_year, is_slip_class		// 요구년도, 수입/지출구분
integer				ii_acct_class							// 회계단위
integer				ii_stat	=	11							// 상태(단위부서신청)
...
if isnumber(string(ld_total_amt)) then	idw_mast.SetItem(idw_mast.getrow(), 'req_amt',   	ld_total_amt)
...
idw_print1.retrieve(is_bdgt_year, is_gwa, ii_acct_class, is_slip_class, ii_bdgt_class)
...
is_slip_class = dw_con.object.slip_class[dw_con.getrow()]
...
idw_mast.setitem(li_newrow, 'worker',		gs_empcode)  // gstru_uid_uname.uid)
idw_mast.setitem(li_newrow, 'ipaddr',		gs_ip)   // gstru_uid_uname.address)
...
if idw_mast.rowcount() > 0 and idw_data.rowcount() < 1 then
	messagebox('확인', '산출근거는 반드시 입력하시기 바랍니다.')
	return -1
else
...
//idw_print1	=	tab_sheet.tabpage_sheet04.dw_print1
//
...
//
//if gi_dept_opt <> 1 then		//예산부서가 아니면...
//	if gs_DeptCode = '2902' then	//전산시스템운영팀...
...
//elseif li_tab = 4 then
//	IF idw_print1.RowCount() < 1 THEN	return
//	
...
if gi_dept_opt <> 1 then		//예산부서가 아니면...
	if gs_DeptCode = '2902' or gs_empcode = 'admin' then	//전산시스템운영팀...
		dw_con.setitem(1, 'code', '')
		is_gwa = ''
	else
...
type ln_templeft from w_tabsheet`ln_templeft within w_hac201a
end type

type ln_tempright from w_tabsheet`ln_tempright within w_hac201a
end type

type ln_temptop from w_tabsheet`ln_temptop within w_hac201a
end type

type ln_tempbuttom from w_tabsheet`ln_tempbuttom within w_hac201a
end type

type ln_tempbutton from w_tabsheet`ln_tempbutton within w_hac201a
end type

type ln_tempstart from w_tabsheet`ln_tempstart within w_hac201a
end type

type uc_retrieve from w_tabsheet`uc_retrieve within w_hac201a
end type

type uc_insert from w_tabsheet`uc_insert within w_hac201a
end type

type uc_delete from w_tabsheet`uc_delete within w_hac201a
end type

type uc_save from w_tabsheet`uc_save within w_hac201a
end type

type uc_excel from w_tabsheet`uc_excel within w_hac201a
end type

type uc_print from w_tabsheet`uc_print within w_hac201a
end type

type st_line1 from w_tabsheet`st_line1 within w_hac201a
...
type st_line2 from w_tabsheet`st_line2 within w_hac201a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type st_line3 from w_tabsheet`st_line3 within w_hac201a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type uc_excelroad from w_tabsheet`uc_excelroad within w_hac201a
end type

type ln_dwcon from w_tabsheet`ln_dwcon within w_hac201a
end type

type tab_sheet from w_tabsheet`tab_sheet within w_hac201a
integer x = 59
integer y = 320
integer width = 4370
integer height = 2244
...
Choose case newindex
	case 4 
		idw_print = tab_sheet.tabpage_sheet04.dw_print1
	case Else
		idw_print = tab_sheet.tabpage_sheet03.dw_print
End Choose
...
if isnull(getitemstring(currentrow, 'hac009h_work_gbn')) and isnull(getitemstring(currentrow, 'hac011h_work_gbn'))then
	is_mast_work_gbn  =	''
else
	is_mast_work_gbn	=	'C'
end if
...
//selectrow(0, false)
if ii_mast_stat_class <> ii_stat_class or not isnull(getitemstring(currentrow, 'hac009h_work_gbn')) or not isnull(getitemstring(currentrow, 'hac011h_work_gbn'))then
...
if isnull(getitemstring(newrow, 'hac009h_work_gbn')) and isnull(getitemstring(newrow, 'hac011h_work_gbn'))then
	is_mast_work_gbn  =	''
else
	is_mast_work_gbn	=	'C'
end if
...
//selectrow(0, false)
if ii_mast_stat_class <> ii_stat_class or not isnull(getitemstring(newrow, 'hac009h_work_gbn')) or not isnull(getitemstring(newrow, 'hac011h_work_gbn'))then
...
type uo_tab from w_tabsheet`uo_tab within w_hac201a
integer x = 1509
integer y = 312
end type

type dw_con from w_tabsheet`dw_con within w_hac201a
...
type st_con from w_tabsheet`st_con within w_hac201a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
event itemchanged;call super::itemchanged;dec{0}	ld_val, ld_total_amt
integer	li_str_tab, i, li_end_tab, li_tab
string	ls_data

integer	li_cnt
string	ls_data2

if dwo.name = 'calc_remark' then
	accepttext()
