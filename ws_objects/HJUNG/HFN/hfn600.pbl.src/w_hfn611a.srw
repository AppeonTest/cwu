$PBExportHeader$w_hfn611a.srw
$PBExportComments$[산학협력단]기타소득자 관리/출력
forward
global type w_hfn611a from w_tabsheet
end type
...
type cbx_1 from checkbox within w_hfn611a
end type
end forward

global type w_hfn611a from w_tabsheet
string title = "기타소득자 등록/출력"
cbx_1 cbx_1
end type
global w_hfn611a w_hfn611a
...
public function decimal wf_won_del (decimal adec_won)
end prototypes
...
ls_yyyymm = String(dw_con.object.bdgt_year[1], 'yyyymm')
...
// 수정사항 :
// ==========================================================================================
dec{0}	ldec_pay_amt, ldec_need_amt, ldec_income_amt, ldec_tax_amt, ldec_jumin_amt, ldec_rate
long		ll_row
...
ldec_need_amt    = idw_data.getitemnumber(al_row, 'need_amt')
ldec_income_amt  = idw_data.getitemnumber(al_row, 'income_amt')
...
if isnull(ldec_need_amt) 	then ldec_need_amt 	= 0
if isnull(ldec_rate) 		then ldec_rate 		= 0
if isnull(ldec_tax_amt) 	then ldec_tax_amt	 	= 0

choose case as_colname
	case 'pay_amt'
		ldec_need_amt   = round(ldec_pay_amt * 0.80,0)
		ldec_income_amt = ldec_pay_amt - ldec_need_amt
		ldec_tax_amt    = round(ldec_income_amt * ldec_rate / 100,0)
		ldec_tax_amt    = wf_won_del(ldec_tax_amt)
		ldec_jumin_amt  = round(ldec_tax_amt * 0.1,0)
		ldec_jumin_amt  = wf_won_del(ldec_jumin_amt)

		idw_data.setitem(al_row, 'need_amt', 	ldec_need_amt)
		idw_data.setitem(al_row, 'income_amt', ldec_income_amt)
		idw_data.setitem(al_row, 'tax_amt', 	ldec_tax_amt)
		idw_data.setitem(al_row, 'jumin_amt', 	ldec_jumin_amt)

		idw_list.setitem(ll_row, 'need_amt', 	ldec_need_amt)
		idw_list.setitem(ll_row, 'income_amt', ldec_income_amt)
...
public function decimal wf_won_del (decimal adec_won);decimal	ldec_return

//원단위 버림
ldec_return = truncate(adec_won / 10,0) * 10	
...
idw_data.setitem(li_newrow, 'bal_gubun',		'1')
idw_data.setitem(li_newrow, 'geju_gubun',		'Y')
idw_data.setitem(li_newrow, 'income_rate',	20)
...
idw_list.setitem(li_newrow2, 'bal_gubun',		'1')
idw_list.setitem(li_newrow2, 'geju_gubun',	'Y')
idw_list.setitem(li_newrow2, 'income_rate',	20)
...
//
//tab_sheet.tabpage_sheet02.uo_4.st_remark.text = '소득자명으로 자료를 조회합니다.'
//tab_sheet.tabpage_sheet02.uo_4.rb_1.text		 =	'소득자명'
//tab_sheet.tabpage_sheet02.uo_4.rb_2.visible	 =	false
//tab_sheet.tabpage_sheet02.uo_4.uf_reset(idw_preview, 'income_name', 'income_name')
//
end event
...
INTO		:LL_SEQ_NO
FROM		FNDB.HFN301H_INDUSTRY
WHERE		YYYYMM = :ls_yyyymm	;

for ll_cnt = 1 to idw_list.rowcount()
	if isnull(idw_list.getitemnumber(ll_cnt, 'seq_no')) or idw_list.getitemnumber(ll_cnt, 'seq_no') = 0 then
		idw_list.setitem(ll_cnt, 'seq_no', ll_seq_no)
	end if
...
avc_data.SetProperty('title', "기타소득자내역")
...
type ln_templeft from w_tabsheet`ln_templeft within w_hfn611a
end type

type ln_tempright from w_tabsheet`ln_tempright within w_hfn611a
end type

type ln_temptop from w_tabsheet`ln_temptop within w_hfn611a
end type

type ln_tempbuttom from w_tabsheet`ln_tempbuttom within w_hfn611a
end type

type ln_tempbutton from w_tabsheet`ln_tempbutton within w_hfn611a
end type

type ln_tempstart from w_tabsheet`ln_tempstart within w_hfn611a
end type

type uc_retrieve from w_tabsheet`uc_retrieve within w_hfn611a
end type

type uc_insert from w_tabsheet`uc_insert within w_hfn611a
end type

type uc_delete from w_tabsheet`uc_delete within w_hfn611a
end type

type uc_save from w_tabsheet`uc_save within w_hfn611a
end type

type uc_excel from w_tabsheet`uc_excel within w_hfn611a
end type

type uc_print from w_tabsheet`uc_print within w_hfn611a
end type

type st_line1 from w_tabsheet`st_line1 within w_hfn611a
...
type st_line2 from w_tabsheet`st_line2 within w_hfn611a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type st_line3 from w_tabsheet`st_line3 within w_hfn611a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type uc_excelroad from w_tabsheet`uc_excelroad within w_hfn611a
end type

type ln_dwcon from w_tabsheet`ln_dwcon within w_hfn611a
end type

type tab_sheet from w_tabsheet`tab_sheet within w_hfn611a
integer y = 316
integer width = 4384
integer height = 2244
integer textsize = -9
...
type uo_tab from w_tabsheet`uo_tab within w_hfn611a
long backcolor = 1073741824
end type

type dw_con from w_tabsheet`dw_con within w_hfn611a
string dataobject = "d_hfn537p_con"
end type

event dw_con::constructor;call super::constructor;This.object.bdgt_year_t.text = '소득년월'

end event

type st_con from w_tabsheet`st_con within w_hfn611a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
choose case dwo.name
	case 'jumin_no'
		if not isnull(data) and trim(data) <> '' then
			...
			if ll_cnt > 0 then
				openwithparm(w_hfn611a_help, '1' + data)
	...
	case 'income_name'
		if not isnull(data) and trim(data) <> '' then
			...
			if ll_cnt > 0 then
				openwithparm(w_hfn611a_help, '2' + data)
	...
	case 'pay_amt', 'need_amt', 'income_amt', 'income_rate', 'tax_amt'
