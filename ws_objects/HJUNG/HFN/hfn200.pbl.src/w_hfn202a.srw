$PBExportHeader$w_hfn202a.srw
$PBExportComments$결의서송부
forward
global type w_hfn202a from w_msheet
end type
type dw_update from cuo_dwwindow_one_hin within w_hfn202a
end type
type cb_preview from commandbutton within w_hfn202a
end type
type dw_con from uo_dwfree within w_hfn202a
end type
type cb_all from uo_imgbtn within w_hfn202a
end type
type cb_can from uo_imgbtn within w_hfn202a
end type
end forward
...
//// 결의구분 입력여부 체크
//is_SlipClass = MID(TRIM(ddlb_slip_class.Text),1,1)
//
//IF is_SlipClass = '0' OR isNull(is_SlipClass) THEN
//	is_SlipClass = ''
//END IF
//
...
ls_PassDt    = String(ldt_WorkDate,'YYYYMMDDHHMMSS')	//송부일자
li_StepOpt   = 2													//결의상태
ls_StepOptNm = '송부'											//결의상태명
...
DO WHILE ll_Row > 0
	...
	IF li_Chk = 1 THEN
   	...
   	dw_update.Object.com_step_opt_nm[ll_row] = ls_StepOptNm		//결의상태
...
if gs_empcode = 'F0018' or gs_empcode = 'admin' then
...
type ln_templeft from w_msheet`ln_templeft within w_hfn202a
end type

type ln_tempright from w_msheet`ln_tempright within w_hfn202a
end type

type ln_temptop from w_msheet`ln_temptop within w_hfn202a
end type

type ln_tempbuttom from w_msheet`ln_tempbuttom within w_hfn202a
end type

type ln_tempbutton from w_msheet`ln_tempbutton within w_hfn202a
end type

type ln_tempstart from w_msheet`ln_tempstart within w_hfn202a
end type

type uc_retrieve from w_msheet`uc_retrieve within w_hfn202a
end type

type uc_insert from w_msheet`uc_insert within w_hfn202a
end type

type uc_delete from w_msheet`uc_delete within w_hfn202a
end type

type uc_save from w_msheet`uc_save within w_hfn202a
end type

type uc_excel from w_msheet`uc_excel within w_hfn202a
end type

type uc_print from w_msheet`uc_print within w_hfn202a
end type

type st_line1 from w_msheet`st_line1 within w_hfn202a
end type

type st_line2 from w_msheet`st_line2 within w_hfn202a
end type

type st_line3 from w_msheet`st_line3 within w_hfn202a
end type

type uc_excelroad from w_msheet`uc_excelroad within w_hfn202a
end type

type ln_dwcon from w_msheet`ln_dwcon within w_hfn202a
end type
...
IF ll_RowCnt = 0 THEN
	ls_Msg = '결의서 송부대상 자료를 조회 후 사용하시기 바랍니다.'
	wf_SetMsg(ls_Msg)
	MessageBox('확인',ls_Msg)
	RETURN
END IF
