$PBExportHeader$w_hfn106a.srw
$PBExportComments$기타소득자 관리/출력
forward
global type w_hfn106a from w_tabsheet
end type
...
type cbx_1 from checkbox within w_hfn106a
end type
end forward
...
choose case as_colname
	case 'pay_amt'
//		ldec_need_amt   = round(ldec_pay_amt * 0.75,0)
...
//if not isdate(mid(is_yyyymm,1,4) + '/' + mid(is_yyyymm,5,2) + '/01') then
...
type ln_templeft from w_tabsheet`ln_templeft within w_hfn106a
end type

type ln_tempright from w_tabsheet`ln_tempright within w_hfn106a
end type

type ln_temptop from w_tabsheet`ln_temptop within w_hfn106a
end type

type ln_tempbuttom from w_tabsheet`ln_tempbuttom within w_hfn106a
end type

type ln_tempbutton from w_tabsheet`ln_tempbutton within w_hfn106a
end type

type ln_tempstart from w_tabsheet`ln_tempstart within w_hfn106a
end type

type uc_retrieve from w_tabsheet`uc_retrieve within w_hfn106a
end type

type uc_insert from w_tabsheet`uc_insert within w_hfn106a
end type

type uc_delete from w_tabsheet`uc_delete within w_hfn106a
end type

type uc_save from w_tabsheet`uc_save within w_hfn106a
end type

type uc_excel from w_tabsheet`uc_excel within w_hfn106a
end type

type uc_print from w_tabsheet`uc_print within w_hfn106a
end type

type st_line1 from w_tabsheet`st_line1 within w_hfn106a
...
type st_line2 from w_tabsheet`st_line2 within w_hfn106a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type st_line3 from w_tabsheet`st_line3 within w_hfn106a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type uc_excelroad from w_tabsheet`uc_excelroad within w_hfn106a
end type

type ln_dwcon from w_tabsheet`ln_dwcon within w_hfn106a
end type

type tab_sheet from w_tabsheet`tab_sheet within w_hfn106a
integer y = 368
integer width = 4384
integer height = 2244
integer textsize = -9
...
type uo_tab from w_tabsheet`uo_tab within w_hfn106a
long backcolor = 1073741824
end type

type dw_con from w_tabsheet`dw_con within w_hfn106a
...
type st_con from w_tabsheet`st_con within w_hfn106a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
choose case dwo.name
	case 'jumin_no'
		if not isnull(data) and trim(data) <> '' then
			...
			if ll_cnt > 0 then
				openwithparm(w_hfn106a_help, '1' + data)
	...
	case 'income_name'
		if not isnull(data) and trim(data) <> '' then
			...
			if ll_cnt > 0 then
				openwithparm(w_hfn106a_help, '2' + data)
