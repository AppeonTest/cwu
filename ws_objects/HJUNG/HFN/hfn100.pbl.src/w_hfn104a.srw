$PBExportHeader$w_hfn104a.srw
$PBExportComments$계좌 관리/출력
forward
global type w_hfn104a from w_tabsheet
end type
...
global type w_hfn104a from w_tabsheet
string title = "계좌 등록/출력"
end type
global w_hfn104a w_hfn104a
...
public function integer wf_dup_chk (long al_row, string as_acct_no)
...
SELECT	COUNT(*)	INTO	:LL_ROW	FROM	FNDB.HFN003M
WHERE		ACCT_NO = :AS_ACCT_NO ;

if ll_row > 0 then
	messagebox('확인', '이미 등록된 계좌번호입니다.')
	return 1
end if

ll_row = idw_list.find("acct_no = '" + as_acct_no + "'", 1, al_row - 1)
...
ll_row = idw_list.find("acct_no = '" + as_acct_no + "'", al_row + 1, idw_list.rowcount())
...
integer	li_tab
long		ll_row
Integer	li_bank_code

li_tab  = tab_sheet.selectedtab

dw_con.accepttext()
li_bank_code = integer(dw_con.object.code[1])
wf_getchild()
...
if idw_list.retrieve(gi_acct_class, li_bank_code) > 0 then
	idw_list.scrolltorow(ll_row)
	idw_preview.retrieve(gi_acct_class, li_bank_code)
	if idw_print.retrieve(gi_acct_class, li_bank_code) > 0 then
...
// 은행명
idw_list.getchild('bank_code', idw_child)
...
// 회계구분
idw_data.getchild('acct_gubun', idw_child)
idw_child.settransobject(sqlca)
...
// 자금구분
idw_data.getchild('jagum_gubun', idw_child)
idw_child.settransobject(sqlca)
...
// 예금종류
idw_data.getchild('depo_class', idw_child)
idw_child.settransobject(sqlca)
...
idw_preview.getchild('depo_class', idw_child)
idw_child.settransobject(sqlca)
if idw_child.retrieve('depo_class', 0) < 1 then
	idw_child.reset()
	idw_child.insertrow(0)
...
idw_print.getchild('depo_class', idw_child)
idw_child.settransobject(sqlca)
if idw_child.retrieve('depo_class', 0) < 1 then
	idw_child.reset()
	idw_child.insertrow(0)
...
if li_bank_code <> 0	then
	idw_list.setitem(li_newrow, 'bank_code',	li_bank_code)
end if
...
if li_bank_code <> 0	then
	idw_list.setitem(li_newrow2, 'bank_code',	li_bank_code)
end if
idw_list.setitem(li_newrow2, 'use_yn',			'Y')
...
//
//
//f_getdwcommon(dw_con, 'bank_code', 0, 750)
//dw_con.setitem(1, 'code', '0')
//
//tab_sheet.tabpage_sheet01.uo_3.st_remark.text = '계좌번호 또는 예금명으로 자료를 조회합니다.'
//tab_sheet.tabpage_sheet01.uo_3.rb_1.text		 =	'계좌번호'
//tab_sheet.tabpage_sheet01.uo_3.rb_2.text		 =	'예금명'
//tab_sheet.tabpage_sheet01.uo_3.uf_reset(idw_list, 'acct_no', 'depo_name')
//
//tab_sheet.tabpage_sheet02.uo_4.st_remark.text = '계좌번호 또는 예금명으로 자료를 조회합니다.'
//tab_sheet.tabpage_sheet02.uo_4.rb_1.text		 =	'계좌번호'
//tab_sheet.tabpage_sheet02.uo_4.rb_2.text		 =	'예금명'
//tab_sheet.tabpage_sheet02.uo_4.uf_reset(idw_preview, 'acct_no', 'depo_name')
//
end event
...
type ln_templeft from w_tabsheet`ln_templeft within w_hfn104a
end type

type ln_tempright from w_tabsheet`ln_tempright within w_hfn104a
end type

type ln_temptop from w_tabsheet`ln_temptop within w_hfn104a
end type

type ln_tempbuttom from w_tabsheet`ln_tempbuttom within w_hfn104a
end type

type ln_tempbutton from w_tabsheet`ln_tempbutton within w_hfn104a
end type

type ln_tempstart from w_tabsheet`ln_tempstart within w_hfn104a
end type

type uc_retrieve from w_tabsheet`uc_retrieve within w_hfn104a
end type

type uc_insert from w_tabsheet`uc_insert within w_hfn104a
end type

type uc_delete from w_tabsheet`uc_delete within w_hfn104a
end type

type uc_save from w_tabsheet`uc_save within w_hfn104a
end type

type uc_excel from w_tabsheet`uc_excel within w_hfn104a
end type

type uc_print from w_tabsheet`uc_print within w_hfn104a
end type

type st_line1 from w_tabsheet`st_line1 within w_hfn104a
...
type st_line2 from w_tabsheet`st_line2 within w_hfn104a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type st_line3 from w_tabsheet`st_line3 within w_hfn104a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type uc_excelroad from w_tabsheet`uc_excelroad within w_hfn104a
end type

type ln_dwcon from w_tabsheet`ln_dwcon within w_hfn104a
end type

type tab_sheet from w_tabsheet`tab_sheet within w_hfn104a
integer x = 55
integer y = 320
integer width = 4384
integer height = 1972
...
type uo_tab from w_tabsheet`uo_tab within w_hfn104a
long backcolor = 1073741824
end type

type dw_con from w_tabsheet`dw_con within w_hfn104a
...
type st_con from w_tabsheet`st_con within w_hfn104a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
