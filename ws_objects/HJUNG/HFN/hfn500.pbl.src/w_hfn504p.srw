$PBExportHeader$w_hfn504p.srw
$PBExportComments$자금계산서 출력(년결산용)
forward
global type w_hfn504p from w_hfn_print_form1
end type
type cbx_1 from checkbox within w_hfn504p
end type
end forward
...
public subroutine wf_reset_amt (string as_acct_code, string as_slip_class, decimal adc_acct_amt, string as_oper_gbn)
end prototypes
...
// 처리구분(1:예산, 2:잔액)
If as_type = '1' Then
	if as_bdgt_year = ls_bdgt_year then
...
Else
	...
	Where		decode(:gi_acct_class,0,0,a.acct_class) = :gi_acct_class
...
// 기타유동부채
Ldc_bdgt_2140_Amt = wf_data_proc(as_bdgt_year, '922300', '1')
...
// ==========================================================================================
datawindowchild	ldw_child
string				ls_bef_bdgt_year, ls_bdgt_year, ls_slip_class
...
// 회계기간 설정
Select	a.bdgt_year, 	a.from_date, a.to_date
Into		:ls_bdgt_year, :is_strdate, :is_enddate
From		acdb.hac003m a
Where		a.bdgt_year		=	:ls_bdgt_year
And		a.bdgt_class	=	0
And		a.stat_class	=	0 ;
...
ls_bef_bdgt_year = string(integer(ls_bdgt_year) - 1)


if cbx_1.checked then		//연속용지
	CHOOSE CASE ls_slip_class
		CASE '1'			// 수입
			  dw_print.DataObject = 'd_hfn504p_2_1'
		CASE '2'			// 지출
			  dw_print.DataObject = 'd_hfn504p_3_1'
		CASE ELSE		// 수입/지출
			  dw_print.DataObject = 'd_hfn504p_1_1'
	END CHOOSE
else
	CHOOSE CASE ls_slip_class
		CASE '1'			// 수입
			  dw_print.DataObject = 'd_hfn504p_2'
		CASE '2'			// 지출
			  dw_print.DataObject = 'd_hfn504p_3'
		CASE ELSE		// 수입/지출
			  dw_print.DataObject = 'd_hfn504p_1'
	END CHOOSE
end if
...
dw_print.retrieve(gi_acct_class, ls_bdgt_year, is_strdate, is_enddate)
...
if ls_slip_class = '' or ls_slip_class = '1' then 
	wf_jasan_proc(ls_bdgt_year, '1311', '4426', '1')
	wf_jasan_proc(ls_bdgt_year, '1312', '4426', '1')
	wf_jasan_proc(ls_bdgt_year, '1313', '4426', '1')
	wf_jasan_proc(ls_bdgt_year, '1314', '4426', '1')
	wf_jasan_proc(ls_bdgt_year, '1315', '4426', '1')
	wf_jasan_proc(ls_bdgt_year, '1316', '4426', '1')
	wf_jasan_proc(ls_bdgt_year, '1317', '4426', '1')
	wf_jasan_proc(ls_bdgt_year, '1318', '4426', '1')
	wf_jasan_proc(ls_bdgt_year, '1319', '4426', '1')
	wf_jasan_proc(ls_bdgt_year, '1321', '4426', '1')
end if
...
if ls_slip_class = '' or ls_slip_class = '2' then 
	wf_transfer_proc(ls_bdgt_year, '1312', '1319', '2')	//건물매입비
	wf_transfer_proc(ls_bdgt_year, '1316', '1319', '2')	//차량운반구
end if

//기계기구,집기비품 계정을 건물로 대체한 전표가 있음. (물품관리전환 건물대체)(2005.07.12)
if ls_slip_class = '1' or ls_slip_class = '2' then 	//수입 또는 지출 조회
	wf_transfer_proc(ls_bdgt_year, '1312', '1314', ls_slip_class)	//기계기구
	wf_transfer_proc(ls_bdgt_year, '1312', '1315', ls_slip_class)	//집기비품
else																	//전체 조회
	wf_transfer_proc(ls_bdgt_year, '1312', '1314', '1')	//기계기구
	wf_transfer_proc(ls_bdgt_year, '1312', '1315', '1')	//집기비품
	wf_transfer_proc(ls_bdgt_year, '1312', '1314', '2')	//기계기구
	wf_transfer_proc(ls_bdgt_year, '1312', '1315', '2')	//집기비품
end if
...
if ls_slip_class = '' or ls_slip_class = '2' then 
	wf_jasan_proc(ls_bdgt_year, '5224', '1311', '2')
	wf_jasan_proc(ls_bdgt_year, '5224', '1312', '2')
	wf_jasan_proc(ls_bdgt_year, '5224', '1313', '2')
	wf_jasan_proc(ls_bdgt_year, '5224', '1314', '2')
	wf_jasan_proc(ls_bdgt_year, '5224', '1315', '2')
	wf_jasan_proc(ls_bdgt_year, '5224', '1316', '2')
	wf_jasan_proc(ls_bdgt_year, '5224', '1317', '2')
end if


/* 2008.04.02 : 외화환산이익(손실) 대체전표의 경우 -> 잡수입(이익) 계정에서 금액 보정 */
if ls_slip_class = '1' then
	wf_foreign_proc(ls_bdgt_year, '5421', '5422', '1')	//외화환산이익
elseif ls_slip_class = '2' then
	wf_foreign_proc(ls_bdgt_year, '4422', '4421', '2')	//외화환산손실
else
	wf_foreign_proc(ls_bdgt_year, '5421', '5422', '1')	//외화환산이익
	wf_foreign_proc(ls_bdgt_year, '4422', '4421', '2')	//외화환산손실
end if	
...
//금액 리셋...
wf_reset_amt(as_acct_code1, as_slip_class, ldc_acct_amt, '-')
...
OPEN	proc_cur;
DO WHILE TRUE
	FETCH NEXT proc_cur	INTO	:ldc_amt,	:ls_acct_code	;
...
public subroutine wf_reset_amt (string as_acct_code, string as_slip_class, decimal adc_acct_amt, string as_oper_gbn);datawindowchild	ldw_child
long		ll_row
decimal	ldc_acct_amt
String	ls_slip_class
dw_con.accepttext()
ls_slip_class = dw_con.object.slip_class[1]
If ls_slip_class = '%' Then ls_slip_class = ''
...
type ln_templeft from w_hfn_print_form1`ln_templeft within w_hfn504p
end type

type ln_tempright from w_hfn_print_form1`ln_tempright within w_hfn504p
end type

type ln_temptop from w_hfn_print_form1`ln_temptop within w_hfn504p
end type

type ln_tempbuttom from w_hfn_print_form1`ln_tempbuttom within w_hfn504p
end type

type ln_tempbutton from w_hfn_print_form1`ln_tempbutton within w_hfn504p
end type

type ln_tempstart from w_hfn_print_form1`ln_tempstart within w_hfn504p
end type

type uc_retrieve from w_hfn_print_form1`uc_retrieve within w_hfn504p
end type

type uc_insert from w_hfn_print_form1`uc_insert within w_hfn504p
end type

type uc_delete from w_hfn_print_form1`uc_delete within w_hfn504p
end type

type uc_save from w_hfn_print_form1`uc_save within w_hfn504p
end type

type uc_excel from w_hfn_print_form1`uc_excel within w_hfn504p
end type

type uc_print from w_hfn_print_form1`uc_print within w_hfn504p
end type

type st_line1 from w_hfn_print_form1`st_line1 within w_hfn504p
end type

type st_line2 from w_hfn_print_form1`st_line2 within w_hfn504p
end type

type st_line3 from w_hfn_print_form1`st_line3 within w_hfn504p
end type

type uc_excelroad from w_hfn_print_form1`uc_excelroad within w_hfn504p
end type

type ln_dwcon from w_hfn_print_form1`ln_dwcon within w_hfn504p
end type

type st_1 from w_hfn_print_form1`st_1 within w_hfn504p
boolean visible = false
integer x = 2286
integer y = 100
end type

type uo_slip_class from w_hfn_print_form1`uo_slip_class within w_hfn504p
boolean visible = false
integer x = 823
integer y = 72
end type

type uo_acct_class from w_hfn_print_form1`uo_acct_class within w_hfn504p
boolean visible = false
integer x = 992
integer y = 820
end type

type uo_year from w_hfn_print_form1`uo_year within w_hfn504p
boolean visible = false
integer x = 2871
end type

type dw_print from w_hfn_print_form1`dw_print within w_hfn504p
integer height = 2008
string dataobject = "d_hfn504p_1"
boolean border = false
borderstyle borderstyle = stylebox!
end type

type st_2 from w_hfn_print_form1`st_2 within w_hfn504p
boolean visible = false
integer x = 1893
integer y = 100
integer width = 443
...
type dw_con from w_hfn_print_form1`dw_con within w_hfn504p
string dataobject = "d_hfn504p_con"
end type
...
event constructor;if gs_DeptCode = '2902' then
	this.visible = true
	this.checked = true
else
	this.visible = false
