$PBExportHeader$w_hfn508p.srw
$PBExportComments$대차대조표 출력(월결산용)
forward
global type w_hfn508p from w_hfn_print_form2
end type
end forward
...
if ls_all_printyn = 'Y'  then		//연속용지
	dw_print.DataObject = 'd_hfn508p_1_1'
else
	dw_print.DataObject = 'd_hfn508p_1'
end if
...
//7000번대 수입계정코드로 인해서 수정(자산에 합산하기 위해) : 2004.12.23 - Jung Kwang Hoon
...
select 	b.year_month, b.acct_class, b.form_class, b.form_code, b.acct_code, b.form_name,
		sum(b.amt_1), sum(b.amt_2), sum(b.amt_3), sum(b.amt_4), 
		:gs_empcode, :gs_ip, sysdate, :gs_empcode, :gs_ip, sysdate
from 
(
	select	:is_yearmonth	year_month, 
			:ii_acct_class	acct_class, 
			:is_form_class	form_class, 
			decode(substr(acct_code,1,1), '7', '1'||substr(form_code,2,5), form_code) form_code, 
			decode(substr(acct_code,1,1), '7', '1'||substr(acct_code,2,3), acct_code) acct_code, 
			form_name,
			(	select	case	
								when		a.location	=	0
...
//				and		acct_code	<=	a.end_code	)	as	amt_1,
				and		decode(substr(acct_code,1,1), '7', '1'||substr(acct_code,2,5), acct_code)	>=	a.str_code
				and		decode(substr(acct_code,1,1), '7', '1'||substr(acct_code,2,5), acct_code)	<=	a.end_code	)	as	amt_1,
			(	select	case
								when		a.location	=	0
									then	nvl(sum(cr_cash_amt), 0) + nvl(sum(cr_alt_amt), 0)
								when		a.location	=	11
...
//				and		acct_code	<=	a.end_code	)	as	amt_2,
				and		decode(substr(acct_code,1,1), '7', '1'||substr(acct_code,2,5), acct_code)	>=	a.str_code
				and		decode(substr(acct_code,1,1), '7', '1'||substr(acct_code,2,5), acct_code)	<=	a.end_code	)	as	amt_2,
			(	select	case
								when		a.location	=	0
									then	nvl(sum(dr_cash_amt), 0) + nvl(sum(dr_alt_amt), 0)
								when		a.location	=	11
...
//				and		acct_code	<=	a.end_code	)	as	amt_3,
				and		decode(substr(acct_code,1,1), '7', '1'||substr(acct_code,2,5), acct_code)	>=	a.str_code
				and		decode(substr(acct_code,1,1), '7', '1'||substr(acct_code,2,5), acct_code)	<=	a.end_code	)	as	amt_3,
			(	select	case
								when		a.location	=	0
									then	nvl(sum(cr_cash_amt), 0) + nvl(sum(cr_alt_amt), 0)
								when		a.location	=	11
...
//				and		acct_code	<=	a.end_code	)	as	amt_4,
				and		decode(substr(acct_code,1,1), '7', '1'||substr(acct_code,2,5), acct_code)	>=	a.str_code
				and		decode(substr(acct_code,1,1), '7', '1'||substr(acct_code,2,5), acct_code)	<=	a.end_code	)	as	amt_4,
			:gs_empcode, :gs_ip, sysdate, :gs_empcode, :gs_ip, sysdate
	from	acdb.hac006m a
...
group by b.year_month, b.acct_class, b.form_class, b.form_code, b.acct_code, b.form_name,
		:gs_empcode, :gs_ip, sysdate, :gs_empcode, :gs_ip, sysdate
;
...
for i = 1 to ll_rowcnt
	...
	for j = 1 to 5
		...
		// 금년도 자료계산
		select	case	when	substr(acct_code,1,1) in ('1', '4')
					then	(nvl(sum(dr_cash_amt), 0) + nvl(sum(dr_alt_amt), 0)) - (nvl(sum(cr_cash_amt), 0) + nvl(sum(cr_alt_amt), 0))
				else		(nvl(sum(cr_cash_amt), 0) + nvl(sum(cr_alt_amt), 0)) - (nvl(sum(dr_cash_amt), 0) + nvl(sum(dr_alt_amt), 0))
				end
		into	:ldb_amt[j]
		from	fndb.hfn502h
		where	decode(:ii_acct_class, 0, :ii_acct_class, acct_class)	=	:ii_acct_class
		...
		group by substr(acct_code,1,1) ;

		if sqlca.sqlcode <> 0 then	ldb_amt[j] = 0

		// 전년도 자료계산
		select	case	when	substr(acct_code, 2, 1) in ('1', '4')
					then	(nvl(sum(dr_cash_amt), 0) + nvl(sum(dr_alt_amt), 0)) - (nvl(sum(cr_cash_amt), 0) + nvl(sum(cr_alt_amt), 0))
				else		(nvl(sum(cr_cash_amt), 0) + nvl(sum(cr_alt_amt), 0)) - (nvl(sum(dr_cash_amt), 0) + nvl(sum(dr_alt_amt), 0))
				end
		into	:ldb_bef_amt[j]
...
/*	backup... 2004.12.23 : Jung Kwang Hoon
insert	into	fndb.hfn901h
		(	year_month, acct_class, form_class, form_code, acct_code, form_name, 
			amt_1, amt_2, amt_3, amt_4,
			worker, ipaddr, work_date, 
...
type ln_templeft from w_hfn_print_form2`ln_templeft within w_hfn508p
end type

type ln_tempright from w_hfn_print_form2`ln_tempright within w_hfn508p
end type

type ln_temptop from w_hfn_print_form2`ln_temptop within w_hfn508p
end type

type ln_tempbuttom from w_hfn_print_form2`ln_tempbuttom within w_hfn508p
end type

type ln_tempbutton from w_hfn_print_form2`ln_tempbutton within w_hfn508p
end type

type ln_tempstart from w_hfn_print_form2`ln_tempstart within w_hfn508p
end type

type uc_retrieve from w_hfn_print_form2`uc_retrieve within w_hfn508p
end type

type uc_insert from w_hfn_print_form2`uc_insert within w_hfn508p
end type

type uc_delete from w_hfn_print_form2`uc_delete within w_hfn508p
end type

type uc_save from w_hfn_print_form2`uc_save within w_hfn508p
end type

type uc_excel from w_hfn_print_form2`uc_excel within w_hfn508p
end type

type uc_print from w_hfn_print_form2`uc_print within w_hfn508p
end type

type st_line1 from w_hfn_print_form2`st_line1 within w_hfn508p
end type

type st_line2 from w_hfn_print_form2`st_line2 within w_hfn508p
end type

type st_line3 from w_hfn_print_form2`st_line3 within w_hfn508p
end type

type uc_excelroad from w_hfn_print_form2`uc_excelroad within w_hfn508p
end type

type ln_dwcon from w_hfn_print_form2`ln_dwcon within w_hfn508p
end type

type dw_form from w_hfn_print_form2`dw_form within w_hfn508p
integer x = 2427
integer y = 1196
end type

type st_11 from w_hfn_print_form2`st_11 within w_hfn508p
boolean visible = false
integer x = 1207
integer y = 1004
end type

type ddlb_acct_class from w_hfn_print_form2`ddlb_acct_class within w_hfn508p
boolean visible = false
integer x = 1513
integer y = 988
end type

type uo_slip_class from w_hfn_print_form2`uo_slip_class within w_hfn508p
boolean visible = false
integer x = 2190
integer y = 652
...
type uo_acct_class from w_hfn_print_form2`uo_acct_class within w_hfn508p
integer x = 1001
integer y = 768
end type

type uo_year from w_hfn_print_form2`uo_year within w_hfn508p
boolean visible = false
integer x = 2030
integer y = 80
end type

type dw_print from w_hfn_print_form2`dw_print within w_hfn508p
integer height = 2272
string dataobject = "d_hfn508p_1"
end type

type dw_con from w_hfn_print_form2`dw_con within w_hfn508p
