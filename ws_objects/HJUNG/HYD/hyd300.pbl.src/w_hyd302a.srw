$PBExportHeader$w_hyd302a.srw
$PBExportComments$연구업적평가생성및조정
forward
global type w_hyd302a from w_window
end type
type dw_main from uo_grid within w_hyd302a
end type
type dw_con from uo_dw within w_hyd302a
end type
type p_1 from picture within w_hyd302a
end type
type st_main from statictext within w_hyd302a
end type
type uo_create from uo_imgbtn within w_hyd302a
end type
end forward
...
public function integer wf_check_con ();String	ls_evl_year, ls_jikwi_code
Integer li_row
...
type ln_templeft from w_window`ln_templeft within w_hyd302a
end type

type ln_tempright from w_window`ln_tempright within w_hyd302a
end type

type ln_temptop from w_window`ln_temptop within w_hyd302a
end type

type ln_tempbuttom from w_window`ln_tempbuttom within w_hyd302a
end type

type ln_tempbutton from w_window`ln_tempbutton within w_hyd302a
end type

type ln_tempstart from w_window`ln_tempstart within w_hyd302a
end type

type uc_retrieve from w_window`uc_retrieve within w_hyd302a
end type

type uc_insert from w_window`uc_insert within w_hyd302a
end type

type uc_delete from w_window`uc_delete within w_hyd302a
end type

type uc_save from w_window`uc_save within w_hyd302a
end type

type uc_excel from w_window`uc_excel within w_hyd302a
end type

type uc_print from w_window`uc_print within w_hyd302a
end type

type st_line1 from w_window`st_line1 within w_hyd302a
end type

type st_line2 from w_window`st_line2 within w_hyd302a
end type

type st_line3 from w_window`st_line3 within w_hyd302a
end type

type uc_excelroad from w_window`uc_excelroad within w_hyd302a
end type
...
integer taborder = 10
boolean bringtotop = true
string dataobject = "d_hyd302a_con"
boolean border = false
borderstyle borderstyle = stylebox!
end type
...
If li_cnt > 0 Then
	...
	If rtn = 1 Then
		...
		SELECT :is_evl_year, MEMBER_NO, :is_evl_str, :is_evl_end, GWA, UP_BOJIK_DATE, JIKWI_CODE, :is_aft_jikwi_code, 'N', :gs_empCode, NULL, sysdate, :gs_empCode, NULL, sysdate
		FROM INDB.HIN001M
