$PBExportHeader$w_hyd105a.srw
$PBExportComments$분야별평가항목기준점수등록
forward
global type w_hyd105a from w_window
end type
type dw_list from uo_grid within w_hyd105a
end type
type dw_con from uo_dw within w_hyd105a
end type
type dw_main from uo_grid within w_hyd105a
end type
type uc_row_insert from u_picture within w_hyd105a
end type
type uc_row_delete from u_picture within w_hyd105a
end type
type st_main from statictext within w_hyd105a
end type
type p_1 from picture within w_hyd105a
end type
type p_2 from picture within w_hyd105a
end type
type st_detail from statictext within w_hyd105a
end type
type uc_down from u_picture within w_hyd105a
end type
end forward
...
lvc_data.setProperty('column1', 'area_cd')  	//평가분야
lvc_data.setProperty('key1', 'HYD0001')
lvc_data.setProperty('column2', 'gyeyul_id')  	//평가계열
lvc_data.setProperty('key2', 'group12_code')
...
type ln_templeft from w_window`ln_templeft within w_hyd105a
end type

type ln_tempright from w_window`ln_tempright within w_hyd105a
end type

type ln_temptop from w_window`ln_temptop within w_hyd105a
end type

type ln_tempbuttom from w_window`ln_tempbuttom within w_hyd105a
end type

type ln_tempbutton from w_window`ln_tempbutton within w_hyd105a
end type

type ln_tempstart from w_window`ln_tempstart within w_hyd105a
end type

type uc_retrieve from w_window`uc_retrieve within w_hyd105a
end type

type uc_insert from w_window`uc_insert within w_hyd105a
end type

type uc_delete from w_window`uc_delete within w_hyd105a
end type

type uc_save from w_window`uc_save within w_hyd105a
end type

type uc_excel from w_window`uc_excel within w_hyd105a
end type

type uc_print from w_window`uc_print within w_hyd105a
end type

type st_line1 from w_window`st_line1 within w_hyd105a
end type

type st_line2 from w_window`st_line2 within w_hyd105a
end type

type st_line3 from w_window`st_line3 within w_hyd105a
end type

type uc_excelroad from w_window`uc_excelroad within w_hyd105a
end type
...
event type long ue_retrieve();String		ls_year, ls_area_cd, ls_gyeyul_id
Long		ll_rv
...
ls_area_cd = dw_con.object.area_cd[dw_con.GetRow()]
ls_gyeyul_id = dw_con.object.gyeyul_id[dw_con.GetRow()]
...
If ls_year = '' Or IsNull(ls_year) Then
	...
	Return -1
End If
If ls_area_cd = '' Or IsNull(ls_area_cd) Then
	MessageBox("확인","평가표항목을 구성할 분야를  선택하신후, 작업하십시요")
	dw_con.SetFocus()
	dw_con.SetColumn('area_cd')
	Return -1
End If

dw_main.Reset()
THIS.Retrieve(ls_year)
ll_rv = dw_main.Retrieve(ls_year, ls_area_cd, ls_gyeyul_id)
...
integer taborder = 10
boolean bringtotop = true
string dataobject = "d_hyd105a_con"
boolean border = false
...
event clicked;call super::clicked;Integer 	I, li_find, li_row
String		ls_year, ls_area_cd, ls_gyeyul_id, ls_evl_gb, ls_evl_cd, ls_evl_nm, ls_evl_sub_nm
...
For I = 1 to dw_list.RowCount()
	If dw_list.Object.slt_gb[i] = 'Y' Then
		ls_evl_gb			= dw_list.Object.evl_gb[i]
		ls_evl_cd			= dw_list.Object.evl_cd[i]
		ls_evl_nm		= dw_list.Object.evl_nm[i]
		ls_evl_sub_nm	= dw_list.Object.evl_sub_nm[i]
		
		li_find = 0
		li_find = dw_main.Find("evl_gb = '" + ls_evl_gb + "' and evl_cd = '" + ls_evl_cd + "'", 1, dw_main.RowCount())
		If li_find > 0 Then
		Else
			li_row = dw_main.InsertRow(0)
			dw_main.Object.std_year[li_row] = ls_year
			dw_main.Object.area_cd[li_row] = ls_area_cd
			dw_main.Object.gyeyul_id[li_row] = ls_gyeyul_id
			dw_main.Object.evl_gb[li_row] = ls_evl_gb
			dw_main.Object.evl_cd[li_row] = ls_evl_cd
			dw_main.Object.evl_nm[li_row] = ls_evl_nm
			dw_main.Object.evl_sub_nm[li_row] = ls_evl_sub_nm
		End If
	End If
Next
end event
