$PBExportHeader$w_hyd200_pop.srw
$PBExportComments$[w_popup_inq] 지원비신청 팝업
forward
global type w_hyd200_pop from w_ancresponse
end type
type p_msg from picture within w_hyd200_pop
end type
type st_msg from statictext within w_hyd200_pop
end type
type uc_printpreview from u_picture within w_hyd200_pop
end type
type uc_cancel from u_picture within w_hyd200_pop
end type
type uc_ok from u_picture within w_hyd200_pop
end type
type uc_excelroad from u_picture within w_hyd200_pop
end type
type uc_excel from u_picture within w_hyd200_pop
end type
type uc_save from u_picture within w_hyd200_pop
end type
type uc_delete from u_picture within w_hyd200_pop
end type
type uc_insert from u_picture within w_hyd200_pop
end type
type uc_retrieve from u_picture within w_hyd200_pop
end type
type ln_temptop from line within w_hyd200_pop
end type
type ln_1 from line within w_hyd200_pop
end type
type ln_2 from line within w_hyd200_pop
end type
type ln_3 from line within w_hyd200_pop
end type
type r_backline1 from rectangle within w_hyd200_pop
end type
type r_backline2 from rectangle within w_hyd200_pop
end type
type r_backline3 from rectangle within w_hyd200_pop
end type
type uc_print from u_picture within w_hyd200_pop
end type
type dw_main from datawindow within w_hyd200_pop
end type
end forward
...
global w_hyd200_pop w_hyd200_pop
...
event type long ue_inquiry();String ls_std_ym, ls_member_no, ls_mng_no
Long ll_cnt

ls_std_ym = ivc.getproperty("std_ym")
ls_member_no = ivc.getproperty("member_no")
ls_mng_no = ivc.getproperty("mng_no")

 dw_main.retrieve(ls_std_ym, ls_member_no, ls_mng_no)
...
event type integer ue_save();String ls_std_ym, ls_member_no, ls_mng_no, ls_gb, ls_year, ls_sil_gb
String ls_org_support_yn = 'N', ls_org_support_nm = ''
String	ls_deposit_no = '' , ls_bank_cd = '', ls_no
Long ll_cnt
Long ll_pay_amt = 0, ll_org_support_amt = 0
String	ls_err
...
ls_member_no = ivc.getproperty("member_no")
ls_mng_no = ivc.getproperty("mng_no")
ls_gb =  ivc.getproperty("support_gb")  //지원비구분 (학술 국내,국외 때문에 41,42로 받아옴)
ls_year = left(ls_std_ym, 4)
ls_sil_gb = left(ls_gb, 1)  //실제 지원비구분

dw_main.accepttext()

SELECT :ls_year || TO_CHAR(lpad((to_number(nvl(max(substr(RECEIPT_NO,5,10)), 0))) + 1, 6,'0') )
	INTO :ls_no
	FROM YGDB.HYD310T
	WHERE SUBSTR(STD_YM, 1, 4) = :ls_year
	AND SUPPORT_GB = :ls_sil_gb;
	
	 If isnull(ls_no) Then 
		ls_no =  ls_year + '000001'
	End If

If ls_sil_gb = '5' Then //지식재산권
	
	ll_pay_amt = dw_main.object.pay_amt[dw_main.getrow()]
	ls_deposit_no = dw_main.object.deposit_no[dw_main.getrow()]
	ls_bank_cd = dw_main.object.bank_cd[dw_main.getrow()]

	

	INSERT INTO YGDB.HYD310T
	(  STD_YM ,	MEMBER_NO ,	MNG_NO ,		SUPPORT_GB ,	RECEIPT_NO ,
	REQ_DT ,		REQ_MEMBER_NO ,	ORG_SUPPORT_YN ,	ORG_SUPPORT_NM ,ORG_SUPPORT_AMT ,
	BANK_CD ,	DEPOSIT_NO ,	ACCEPT_GB ,	PAY_AMT ,	PAY_CHK ,
	APPR_DTTM ,	APPR_DVS_CD ,	APPR_CNCL_DTTM ,	APPR_RTRN_CNCL_RSN_CNTN ,	ATTACH_GB ,
	ATTACH_APPR ,	
	WORKER ,		IPADDR ,		WORK_DATE ,	JOB_UID ,	JOB_ADD ,	JOB_DATE)
	SELECT STD_YM, 	MEMBER_NO,	MNG_NO,		:ls_sil_gb,	:ls_no,
	TO_CHAR(SYSDATE, 'YYYYMMDD'),  :gs_empCode,	 :ls_org_support_yn, :ls_org_support_nm, :ll_org_support_amt,
	:ls_bank_cd, :ls_deposit_no, '10',  :ll_pay_amt,  'N', 
	...
	AND MNG_NO = :ls_mng_no
	USING SQLCA;
Elseif ls_sil_gb = '4' Then  //학술발표 
	
	If ls_gb = '42' Then //국외'
	
		ls_org_support_yn = dw_main.object.org_support_yn[dw_main.getrow()]
		ls_org_support_nm = dw_main.object.org_support_nm[dw_main.getrow()] 
		ll_org_support_amt =  dw_main.object.org_support_amt[dw_main.getrow()]
...
If SQLCA.SQLCODE <> 0 Then
	...
	st_msg.Text = "[저장] " + '지원비처리내역 생성 중 에러!'
	RETURN -1
Else
	COMMIT USING SQLCA;
	st_msg.Text = "[저장] " + '지원비처리내역 생성 완료!'
	this.trigger event ue_inquiry()
End If
...
For ll_i = 1 To ll_dw_cnt
	...
	li_rv = func.of_setsysdate	(idw_update[ll_i],	0,	'ins_dt',	'N')
	If li_rv <> 1 Then
		RETURN -1
	End If
	li_rv = func.of_setvalue	(idw_update[ll_i],	0,	'ins_id',	gs_empcode, 'N') 
	If li_rv <> 1 Then
		RETURN -1
	End If
	// 최종수정일, 최종수정자 N: 신규 일때  M: Modify일때 set 0: 전체 row Loop하면서 DataModified인것만 set한다. 
	li_rv = func.of_setsysdate	(idw_update[ll_i],	0,	'upd_dt',	'N')
	If li_rv <> 1 Then
		RETURN -1
	End If
	li_rv = func.of_setvalue	(idw_update[ll_i],	0,	'upd_id',	gs_empcode, 'N') 
	If li_rv <> 1 Then
		RETURN -1
	End If
	li_rv = func.of_setsysdate	(idw_update[ll_i],	0,	'upd_dt',	'M')
	If li_rv <> 1 Then
		RETURN -1
	End If
	li_rv = func.of_setvalue	(idw_update[ll_i],	0,	'upd_id',	gs_empcode, 'M') 
	If li_rv <> 1 Then
		RETURN -1
	End If
...
this.Control[iCurrent+1]=this.p_msg
this.Control[iCurrent+2]=this.st_msg
this.Control[iCurrent+3]=this.uc_printpreview
this.Control[iCurrent+4]=this.uc_cancel
this.Control[iCurrent+5]=this.uc_ok
this.Control[iCurrent+6]=this.uc_excelroad
this.Control[iCurrent+7]=this.uc_excel
this.Control[iCurrent+8]=this.uc_save
this.Control[iCurrent+9]=this.uc_delete
this.Control[iCurrent+10]=this.uc_insert
this.Control[iCurrent+11]=this.uc_retrieve
this.Control[iCurrent+12]=this.ln_temptop
this.Control[iCurrent+13]=this.ln_1
this.Control[iCurrent+14]=this.ln_2
this.Control[iCurrent+15]=this.ln_3
this.Control[iCurrent+16]=this.r_backline1
this.Control[iCurrent+17]=this.r_backline2
this.Control[iCurrent+18]=this.r_backline3
this.Control[iCurrent+19]=this.uc_print
this.Control[iCurrent+20]=this.dw_main
end on

on w_hyd200_pop.destroy
call super::destroy
destroy(this.p_msg)
destroy(this.st_msg)
...
String ls_gb
ls_gb = ivc.getproperty("support_gb")

If ls_gb = '1' Then
	dw_main.dataobject = 'd_hyd200_pop1'
Elseif ls_gb = '41' Then
	dw_main.dataobject = 'd_hyd200_pop3'
Elseif ls_gb = '42' Then
	dw_main.dataobject = 'd_hyd200_pop4'
Elseif ls_gb = '5' Then
	dw_main.dataobject = 'd_hyd200_pop2'
...
lvc_data.setProperty('column1', 'bank_cd')  //은행코드
...
integer taborder = 10
string title = "none"
string dataobject = "d_hyd200_pop2"
