$PBExportHeader$w_hyd204a.srw
$PBExportComments$[w_list_master_detail] 저역서실적내역등록
forward
global type w_hyd204a from w_window
end type
type dw_list from uo_grid within w_hyd204a
end type
type dw_con from uo_dw within w_hyd204a
end type
type dw_main from uo_dw within w_hyd204a
end type
type uc_row_insert from u_picture within w_hyd204a
end type
type uc_row_delete from u_picture within w_hyd204a
end type
type dw_sub from uo_grid within w_hyd204a
end type
type st_main from statictext within w_hyd204a
end type
type st_detail from statictext within w_hyd204a
end type
type p_2 from picture within w_hyd204a
end type
type p_1 from picture within w_hyd204a
end type
type p_3 from picture within w_hyd204a
end type
type st_sub from statictext within w_hyd204a
end type
end forward
...
lvc_data.setProperty('key1', 'HYD1340')
...
lvc_data.setProperty('column1', 'book_dvs_cd')  //저역서구분코드
lvc_data.setProperty('key1', 'HYD1110')
lvc_data.setProperty('column2', 'jnl_dvs_cd')  //발행지구분코드
lvc_data.setProperty('key2', 'HYD1140')
lvc_data.setProperty('column3', 'book_char_cd')  //도서성격코드
lvc_data.setProperty('key3', 'HYD1330')
lvc_data.setProperty('column4', 'rvsn_yn')  //개정여부
lvc_data.setProperty('key4', 'HYD1120')
lvc_data.setProperty('column5', 'mkout_lang_cd')  //작성연어코드
lvc_data.setProperty('key5', 'HYD1130')
...
type ln_templeft from w_window`ln_templeft within w_hyd204a
end type

type ln_tempright from w_window`ln_tempright within w_hyd204a
end type

type ln_temptop from w_window`ln_temptop within w_hyd204a
end type

type ln_tempbuttom from w_window`ln_tempbuttom within w_hyd204a
end type

type ln_tempbutton from w_window`ln_tempbutton within w_hyd204a
end type

type ln_tempstart from w_window`ln_tempstart within w_hyd204a
end type

type uc_retrieve from w_window`uc_retrieve within w_hyd204a
end type

type uc_insert from w_window`uc_insert within w_hyd204a
end type

type uc_delete from w_window`uc_delete within w_hyd204a
end type

type uc_save from w_window`uc_save within w_hyd204a
end type

type uc_excel from w_window`uc_excel within w_hyd204a
end type

type uc_print from w_window`uc_print within w_hyd204a
end type

type st_line1 from w_window`st_line1 within w_hyd204a
end type

type st_line2 from w_window`st_line2 within w_hyd204a
end type

type st_line3 from w_window`st_line3 within w_hyd204a
end type

type uc_excelroad from w_window`uc_excelroad within w_hyd204a
end type
...
If ll_rv > 0 Then
	dw_sub.Retrieve(ls_std_ym, ls_member_no, ls_mng_no, '2')  //참여자명단귀속구분 - 2 : 저역서실적
Else
	This.insertrow(0)
	dw_sub.Reset()
End If
...
Choose Case	dwo.name
	Case 'std_ym'
		this.object.book_pblc_ym[row] = data
...
This.object.include_gb[al_row] = '2'  //저역서실적 참여자 명단 귀속구분

If al_row = 1 then 
	ll_seq_no = 1
	This.object.seq_no[al_row] = ll_seq_no
	This.event itemchanged(al_row, this.object.pcn_rschr_reg_no, gs_empcode)
Else
...
event itemchanged;call super::itemchanged;
String		ls_h01nno	,ls_h01knm, ls_duty_code
...
Choose Case	dwo.name
	Case	'pcn_rschr_reg_no','prtcpnt_nm'
		...
		If func.of_nvl(data,'') = '' Then
			...
			This.Post Setitem(row, 'posi_cd', '')
			RETURN
		End If
		
		SELECT MEMBER_NO, NAME, DUTY_CODE
		INTO :ls_h01nno , :ls_h01knm, :ls_duty_code
		FROM INDB.HIN001M
		WHERE  MEMBER_NO LIKE :ls_h01nno || '%'
		AND NAME LIKE :ls_h01knm || '%'
		USING SQLCA;
		...
		If SQLCA.SQLCODE = 0 AND SQLCA.SQLNROWS = 1 Then
			...
			This.object.posi_cd[row] = ls_duty_code
...
s_insa_com	lstr_com
String		ls_KName, ls_member_no, ls_duty_code
This.accepttext()
ls_KName =  trim(this.object.prtcpnt_nm[row])
...
ls_kname               = lstr_com.ls_item[01]	//성명
ls_Member_No            = lstr_com.ls_item[02]	//개인번호
 ls_duty_code = lstr_com.ls_item[07] 
this.object.prtcpnt_nm[row]        = ls_kname					//성명
This.object.pcn_rschr_reg_no[row]     = ls_Member_No				//개인번호
this.object.posi_cd[row] = ls_duty_code
End If
end event
