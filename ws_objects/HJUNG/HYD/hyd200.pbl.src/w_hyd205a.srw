$PBExportHeader$w_hyd205a.srw
$PBExportComments$[w_list_master_detail] 연구비수혜실적등록
forward
global type w_hyd205a from w_window
end type
type dw_list from uo_grid within w_hyd205a
end type
type dw_con from uo_dw within w_hyd205a
end type
type dw_main from uo_dw within w_hyd205a
end type
type uc_row_insert from u_picture within w_hyd205a
end type
type uc_row_delete from u_picture within w_hyd205a
end type
type dw_sub from uo_grid within w_hyd205a
end type
type st_main from statictext within w_hyd205a
end type
type st_detail from statictext within w_hyd205a
end type
type p_2 from picture within w_hyd205a
end type
type p_1 from picture within w_hyd205a
end type
type p_3 from picture within w_hyd205a
end type
type st_sub from statictext within w_hyd205a
end type
type dw_year from uo_grid within w_hyd205a
end type
type st_1 from statictext within w_hyd205a
end type
type p_4 from picture within w_hyd205a
end type
type uc_row_insert1 from u_picture within w_hyd205a
end type
type uc_row_delete1 from u_picture within w_hyd205a
end type
end forward
...
this.Control[iCurrent+13]=this.dw_year
this.Control[iCurrent+14]=this.st_1
this.Control[iCurrent+15]=this.p_4
this.Control[iCurrent+16]=this.uc_row_insert1
this.Control[iCurrent+17]=this.uc_row_delete1
end on

on w_hyd205a.destroy
call super::destroy
...
lvc_data.setProperty('column1', 'rsrcct_sppt_dvs_cd')  //연구비지원구분
lvc_data.setProperty('key1', 'HYD1280')
lvc_data.setProperty('column2', 'mny_yr_sbjt_yn')  //다년과제여부
lvc_data.setProperty('key2', 'HYD1450')
lvc_data.setProperty('column3', 'rsch_sbjt_stdy_sphe_cd')  //연구과제학문분야
lvc_data.setProperty('key3', 'HYD1350')
func.of_dddw( dw_main,lvc_data)

lvc_data.setProperty('column1', 'appr_dvs_cd')  //승인구분
lvc_data.setProperty('key1', 'HYD1400')
func.of_dddw( dw_year,lvc_data)
...
If uc_row_insert1.Enabled Then
	uc_row_insert1.Visible	= TRUE
Else
	uc_row_insert1.Visible	= FALSE
End If

If uc_row_delete1.Enabled Then
	uc_row_delete1.Visible	= TRUE
Else
	uc_row_delete1.Visible	= FALSE
End If
...
type ln_templeft from w_window`ln_templeft within w_hyd205a
end type

type ln_tempright from w_window`ln_tempright within w_hyd205a
end type

type ln_temptop from w_window`ln_temptop within w_hyd205a
end type

type ln_tempbuttom from w_window`ln_tempbuttom within w_hyd205a
end type

type ln_tempbutton from w_window`ln_tempbutton within w_hyd205a
end type

type ln_tempstart from w_window`ln_tempstart within w_hyd205a
end type

type uc_retrieve from w_window`uc_retrieve within w_hyd205a
end type

type uc_insert from w_window`uc_insert within w_hyd205a
end type

type uc_delete from w_window`uc_delete within w_hyd205a
end type

type uc_save from w_window`uc_save within w_hyd205a
end type

type uc_excel from w_window`uc_excel within w_hyd205a
end type

type uc_print from w_window`uc_print within w_hyd205a
end type

type st_line1 from w_window`st_line1 within w_hyd205a
end type

type st_line2 from w_window`st_line2 within w_hyd205a
end type

type st_line3 from w_window`st_line3 within w_hyd205a
end type

type uc_excelroad from w_window`uc_excelroad within w_hyd205a
end type
...
If currentrow > 0 And (ll_rv = 1 or ll_rv = 2) Then
	If dw_list.GetItemStatus(currentrow, 0, Primary!) <> New! THEN 
		...
		dw_year.Post Event ue_Retrieve()
	Else
...
If ll_rv > 0 Then
	dw_sub.Retrieve(ls_std_ym, ls_member_no, ls_mng_no, '3')  //참여자명단귀속구분 - 3 : 연구비수혜자실적
Else
	This.insertrow(0)
	dw_sub.Reset()
End If
...
This.object.include_gb[al_row] = '3'  //연구비실적 참여자 명단 귀속구분
...
ll_rv = this.Retrieve(ls_std_ym, ls_member_no, ls_mng_no)
...
event clicked;call super::clicked;If dw_list.GetRow() > 0 Then
	dw_year.PostEvent("ue_InsertRow")
End If
...
event clicked;call super::clicked;dw_year.PostEvent("ue_DeleteRow")
