$PBExportHeader$w_hss501a.srw
$PBExportComments$기타구입신청건
forward
global type w_hss501a from w_msheet
end type
type ddlb_gbn from dropdownlistbox within w_hss501a
end type
type st_10 from statictext within w_hss501a
end type
type sle_name from singlelineedit within w_hss501a
end type
type st_3 from statictext within w_hss501a
end type
type tab_1 from tab within w_hss501a
end type
...
type sle_seq from singlelineedit within tabpage_2
end type
type sle_name3 from singlelineedit within tabpage_2
end type
type st_7 from statictext within tabpage_2
end type
type st_6 from statictext within tabpage_2
end type
type em_date from editmask within tabpage_2
end type
...
type st_2 from statictext within w_hss501a
end type
type em_to_date from editmask within w_hss501a
end type
type em_fr_date from editmask within w_hss501a
end type
type st_1 from statictext within w_hss501a
end type
type uo_1 from u_tab within w_hss501a
end type
type gb_1 from groupbox within w_hss501a
end type
end forward
...
tab_1.tabpage_2.em_date.text = ls_Today
...
tab_1.tabpage_1.sle_name2.text = ''
  select max(apply_seq)
  into   :ll_cnt
  from   stdb.hst317h ;
  
	tab_1.tabpage_1.sle_number.text = STring(ll_cnt + 1)
...
CHOOSE CASE ii_tab	
   ...
   CASE 2
		ls_apply_date  = tab_1.tabpage_2.em_date.text
	   ls_apply_date2 = left(ls_apply_date,4) + mid(ls_apply_date,6,2) + right(ls_apply_date,2)
      IF isnull(ls_apply_date2) OR ls_apply_date2 ='' THEN
	      messagebox('확인','신청일자를 입력하시기 바랍니다..!')
      END IF
      
		ls_mid_name2 = trim(tab_1.tabpage_2.sle_name3.text)
      
		IF isnull(ls_mid_name2) OR ls_mid_name2 ='' THEN
	      ls_mid_name2 = '%'
      END IF
      
		ll_apply_seq = long(tab_1.tabpage_2.sle_seq.text)
      
		IF isnull(ll_apply_seq) OR ll_apply_seq = 0 THEN
	      ll_apply_seq = 0
      END IF
		
       tab_1.tabpage_2.dw_print.SetReDraw(FALSE)
       ll_RowCnt = tab_1.tabpage_2.dw_print.Retrieve(ls_apply_date2, ls_mid_name2, ll_apply_seq)
...
event ue_button_set;call super::ue_button_set;tab_1.tabpage_1.cb_2.x = tab_1.tabpage_1.cb_1.x + tab_1.tabpage_1.cb_1.width + 16
end event
...
avc_data.SetProperty('title', "물품구입 요구서")
...
type ln_templeft from w_msheet`ln_templeft within w_hss501a
end type

type ln_tempright from w_msheet`ln_tempright within w_hss501a
end type

type ln_temptop from w_msheet`ln_temptop within w_hss501a
end type

type ln_tempbuttom from w_msheet`ln_tempbuttom within w_hss501a
end type

type ln_tempbutton from w_msheet`ln_tempbutton within w_hss501a
end type

type ln_tempstart from w_msheet`ln_tempstart within w_hss501a
end type

type uc_retrieve from w_msheet`uc_retrieve within w_hss501a
end type

type uc_insert from w_msheet`uc_insert within w_hss501a
end type

type uc_delete from w_msheet`uc_delete within w_hss501a
end type

type uc_save from w_msheet`uc_save within w_hss501a
end type

type uc_excel from w_msheet`uc_excel within w_hss501a
end type

type uc_print from w_msheet`uc_print within w_hss501a
end type

type st_line1 from w_msheet`st_line1 within w_hss501a
...
type st_line2 from w_msheet`st_line2 within w_hss501a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type st_line3 from w_msheet`st_line3 within w_hss501a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type uc_excelroad from w_msheet`uc_excelroad within w_hss501a
end type

type ln_dwcon from w_msheet`ln_dwcon within w_hss501a
end type
...
long  ll_fr_date, li_idx, ll_apply_seq
DataWindow  dw_name
dw_name = tab_1.tabpage_1.dw_main
...
event rowfocuschanging;call super::rowfocuschanging;Long	ll_RowCnt, ll_apply_seq, ll_getrow
...
on tabpage_2.create
this.sle_seq=create sle_seq
this.sle_name3=create sle_name3
...
destroy(this.sle_seq)
destroy(this.sle_name3)
destroy(this.st_7)
destroy(this.st_6)
