$PBExportHeader$w_hss318a.srw
$PBExportComments$방화설비관리
forward
global type w_hss318a from w_msheet
end type
type gb_2 from groupbox within w_hss318a
end type
type tab_1 from tab within w_hss318a
end type
...
type st_2 from statictext within w_hss318a
end type
type em_to_date from editmask within w_hss318a
end type
type em_fr_date from editmask within w_hss318a
end type
type st_1 from statictext within w_hss318a
end type
type gb_1 from groupbox within w_hss318a
end type
type em_set_date from editmask within w_hss318a
end type
type st_3 from statictext within w_hss318a
end type
type uo_1 from u_tab within w_hss318a
end type
type cb_1 from uo_imgbtn within w_hss318a
end type
end forward
...
avc_data.SetProperty('title', "방화설비 리스트")
...
type ln_templeft from w_msheet`ln_templeft within w_hss318a
end type

type ln_tempright from w_msheet`ln_tempright within w_hss318a
end type

type ln_temptop from w_msheet`ln_temptop within w_hss318a
end type

type ln_tempbuttom from w_msheet`ln_tempbuttom within w_hss318a
end type

type ln_tempbutton from w_msheet`ln_tempbutton within w_hss318a
end type

type ln_tempstart from w_msheet`ln_tempstart within w_hss318a
end type

type uc_retrieve from w_msheet`uc_retrieve within w_hss318a
end type

type uc_insert from w_msheet`uc_insert within w_hss318a
end type

type uc_delete from w_msheet`uc_delete within w_hss318a
end type

type uc_save from w_msheet`uc_save within w_hss318a
end type

type uc_excel from w_msheet`uc_excel within w_hss318a
end type

type uc_print from w_msheet`uc_print within w_hss318a
end type

type st_line1 from w_msheet`st_line1 within w_hss318a
...
type st_line2 from w_msheet`st_line2 within w_hss318a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type st_line3 from w_msheet`st_line3 within w_hss318a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type uc_excelroad from w_msheet`uc_excelroad within w_hss318a
end type

type ln_dwcon from w_msheet`ln_dwcon within w_hss318a
end type
...
event clicked;call super::clicked;String  ls_check_date, ls_check_remark, ls_result, ls_remark, ls_place, ls_hangmok
String  ls_set_date, ls_max_date , ls_set_date2, ls_date, ls_checker, ls_comfirm_name
Long    ll_count, ll_seq_no, ll_sunwi
...
IF ll_count > 0 THEN 
   ...
   DECLARE sel_cur CURSOR FOR
    ...
    DO WHILE TRUE
	    FETCH sel_cur INTO :ls_check_date,:ll_seq_no,:ls_place,:ls_hangmok,:ll_sunwi,:ls_checker, &
		                    :ls_comfirm_name,:ls_check_remark,:ls_result,:ls_remark;
			if sqlca.sqlcode <> 0  then  
				 close sel_cur;				
   			return
			End If
		   li_CurrentRow = dw_name.insertrow(0)
			...
			dw_name.object.place[li_CurrentRow]        = ls_place
			dw_name.object.hangmok[li_CurrentRow]      = ls_hangmok
			dw_name.object.sunwi[li_CurrentRow]        = ll_sunwi
			dw_name.object.checker[li_CurrentRow]      = ls_checker
			dw_name.object.comfirm_name[li_CurrentRow] = ls_comfirm_name
			dw_name.object.check_remark[li_CurrentRow] = ls_check_remark
			dw_name.object.result[li_CurrentRow]       = ls_result
