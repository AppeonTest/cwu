$PBExportHeader$w_hst602p.srw
$PBExportComments$학과별 종취득 금액별현황
forward
global type w_hst602p from w_msheet
end type
type dw_select from datawindow within w_hst602p
end type
type gb_1 from groupbox within w_hst602p
end type
type dw_print from datawindow within w_hst602p
end type
end forward
...
string ls_f_date, ls_t_date, ll_amt_f, ll_amt_t, ls_text1,ls_pur_opt_f,ls_pur_opt_t, ls_useful_opt,ls_useful_t
...
if	not isnull(dw_select.object.c_epryygb_f[dw_select.getrow()]) and	&
	...
	ls_statement += " and A.oper_opt >= "+li_oper_opt_f +"	 and  A.oper_opt <= "+li_oper_opt_t
		...
		ls_useful_opt	=	string(dw_select.object.useful_opt[dw_select.getrow()])		
if	not isnull(dw_select.object.useful_opt[dw_select.getrow()])  then
	ls_statement += " and A.useful = "+ls_useful_opt 
end if	
													

	is_sql = dw_print.getsqlselect()
	
	is_sql = Mid(is_sql, 1, Len(is_sql) - (Len(is_sql) - Pos(is_sql, "group by", 350) + 1))
	
	ls_new = is_sql + ls_statement + ' group by a.gwa'
	...
	dw_print.SetSQLSelect(is_sql + ' group by a.gwa')
...
f_childretrieve(dw_select,"useful_opt","useful_opt")		   //구매구분
		
dw_select.reset()
dw_select.insertrow(0)
dw_select.setitem(1, "c_eprbymd_f", (left(f_today(),4)+mid(f_today(),5,2) + '01'))
dw_select.setitem(1, "c_eprbymd_t", f_today())
...
avc_data.SetProperty('title', "학과별 종(점수) 취득금액별 현황")
...
type ln_templeft from w_msheet`ln_templeft within w_hst602p
end type

type ln_tempright from w_msheet`ln_tempright within w_hst602p
end type

type ln_temptop from w_msheet`ln_temptop within w_hst602p
end type

type ln_tempbuttom from w_msheet`ln_tempbuttom within w_hst602p
end type

type ln_tempbutton from w_msheet`ln_tempbutton within w_hst602p
end type

type ln_tempstart from w_msheet`ln_tempstart within w_hst602p
end type

type uc_retrieve from w_msheet`uc_retrieve within w_hst602p
end type

type uc_insert from w_msheet`uc_insert within w_hst602p
end type

type uc_delete from w_msheet`uc_delete within w_hst602p
end type

type uc_save from w_msheet`uc_save within w_hst602p
end type

type uc_excel from w_msheet`uc_excel within w_hst602p
end type

type uc_print from w_msheet`uc_print within w_hst602p
end type

type st_line1 from w_msheet`st_line1 within w_hst602p
...
type st_line2 from w_msheet`st_line2 within w_hst602p
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type st_line3 from w_msheet`st_line3 within w_hst602p
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type uc_excelroad from w_msheet`uc_excelroad within w_hst602p
end type

type ln_dwcon from w_msheet`ln_dwcon within w_hst602p
end type
