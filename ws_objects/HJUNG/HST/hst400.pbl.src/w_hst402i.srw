$PBExportHeader$w_hst402i.srw
$PBExportComments$비품이동처리
forward
global type w_hst402i from w_tabsheet
end type
...
type dw_head from datawindow within w_hst402i
end type
type rb_total from radiobutton within w_hst402i
end type
type rb_dele from radiobutton within w_hst402i
end type
type gb_4 from groupbox within w_hst402i
end type
type gb_1 from groupbox within w_hst402i
end type
end forward
...
//		tab_sheet.tabpage_sheet01.dw_update_tab.Object.Work_Date[ll_Row] =ls_sysdate//등록일자                                                                                                                                    
...
//		tab_sheet.tabpage_sheet01.dw_update_tab.Object.job_date[ll_Row]  =ls_sysdate//작업일자
...
CHOOSE CASE ii_tab
  ...
  CASE 2
		...
		END IF	 
  CASE 3
	   IF tab_sheet.tabpage_1.dw_print1.retrieve( ls_id_no, ls_item_no, ls_item_name, ls_item_class, ls_date_f, ls_date_t, ls_dept_code ) = 0 THEN
			wf_setMsg("조회된 데이타가 없습니다")	
		ELSE	
			wf_SetMsg('자료가 조회되었습니다.')			
         tab_sheet.tabpage_1.dw_print1.setfocus()
			//wf_setmenu('P', TRUE)
		END IF
  CASE 4
...
f_childretrieve(tab_sheet.tabpage_1.dw_print1,"item_class","item_class")        // 물품구분(저장)

f_childretrieve(tab_sheet.tabpage_2.dw_print2,"item_class","item_class")   // 물품구분(저장)

f_childretrieven(tab_sheet.tabpage_sheet01.dw_update_tab,"gwa")
...
CHOOSE CASE ii_tab
	...
	CASE 1
	  ...
	  for i = 1 to idw_name1.rowcount()
			if idw_name1.object.move_opt[i] = 2 then
   ...
   CASE 2
     ...
     FOR i = 1 TO ll_rowcount
		  ...
		  IF idw_name1.object.move_opt[i] = 1 THEN 
				idw_name1.object.move_proc_date[i] = '00000000'
				ll_seq_no     = idw_name1.object.seq_no[i]	
				ls_id_no      = trim(idw_name1.object.id_no[i])
				ll_item_class = idw_name1.object.item_class[i]
				ls_dept       = trim(idw_name1.object.move_bef_gwa[i])
				ls_room       = trim(idw_name1.object.move_bef_room[i])
...
//		
//	CASE 3
//		f_print(tab_sheet.tabpage_1.dw_print1)
//	CASE 4
...
type ln_templeft from w_tabsheet`ln_templeft within w_hst402i
end type

type ln_tempright from w_tabsheet`ln_tempright within w_hst402i
end type

type ln_temptop from w_tabsheet`ln_temptop within w_hst402i
end type

type ln_tempbuttom from w_tabsheet`ln_tempbuttom within w_hst402i
end type

type ln_tempbutton from w_tabsheet`ln_tempbutton within w_hst402i
end type

type ln_tempstart from w_tabsheet`ln_tempstart within w_hst402i
end type

type uc_retrieve from w_tabsheet`uc_retrieve within w_hst402i
end type

type uc_insert from w_tabsheet`uc_insert within w_hst402i
end type

type uc_delete from w_tabsheet`uc_delete within w_hst402i
end type

type uc_save from w_tabsheet`uc_save within w_hst402i
end type

type uc_excel from w_tabsheet`uc_excel within w_hst402i
end type

type uc_print from w_tabsheet`uc_print within w_hst402i
end type

type st_line1 from w_tabsheet`st_line1 within w_hst402i
...
type st_line2 from w_tabsheet`st_line2 within w_hst402i
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type st_line3 from w_tabsheet`st_line3 within w_hst402i
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type uc_excelroad from w_tabsheet`uc_excelroad within w_hst402i
end type

type ln_dwcon from w_tabsheet`ln_dwcon within w_hst402i
end type

type tab_sheet from w_tabsheet`tab_sheet within w_hst402i
integer y = 468
integer width = 4384
integer height = 1836
integer textsize = -9
...
this.Control[iCurrent+2]=this.dw_update3
end on
...
IF dwo.name = 'move_opt' THEN   

   IF data = '2' THEN	             // 처 리
	
		this.object.move_proc_date[row] = f_today()
		this.object.move_proc_member_no[row] = gstru_uid_uname.uid

	ELSE			
		
	   this.object.move_proc_date[row] = '00000000'
		this.object.move_proc_member_no[row] = ''
...
type uo_tab from w_tabsheet`uo_tab within w_hst402i
integer x = 1824
integer y = 460
long backcolor = 1073741824
end type

type dw_con from w_tabsheet`dw_con within w_hst402i
boolean visible = false
end type

type st_con from w_tabsheet`st_con within w_hst402i
boolean visible = false
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
...
boolean bringtotop = true
string title = "none"
string dataobject = "d_hst402i_20"
boolean hscrollbar = true
boolean vscrollbar = true
boolean border = false
...
boolean bringtotop = true
string title = "none"
string dataobject = "d_hst402i_30"
boolean hscrollbar = true
boolean vscrollbar = true
boolean border = false
boolean livescroll = true
