$PBExportHeader$w_hst302i.srw
$PBExportComments$재물조사대상생성
forward
global type w_hst302i from w_msheet
end type
type gb_1 from groupbox within w_hst302i
end type
type uo_1 from u_tab within w_hst302i
end type
type tab_1 from tab within w_hst302i
end type
...
type rb_1 from radiobutton within w_hst302i
end type
type rb_2 from radiobutton within w_hst302i
end type
type dw_head from datawindow within w_hst302i
end type
end forward
...
string	ls_room_name, ls_gwa_name, ls_room_cd_name
...
ls_RoomCd1      = TRIM(dw_head.object.c_room_name     [1]) 		//사용장소
...
ls_room_cd_name = dw_head.Describe("Evaluate('LookUpDisplay(c_room_name)', 1)")

IF 	isNull(ls_DateFr) or  isnull(ls_DateTo)  THEN 
		 
ELSE
		ls_header += 	"등재일자 : "+ls_DateFr+"-"+ls_DateTo+"  "
END IF
...
IF isNull(ls_Gwa)    THEN 
	ls_Gwa    = '%'
ELSE
	ls_gwa_name = dw_head.Describe("Evaluate('LookUpDisplay(c_dept_code) ', 1)")	
...
IF isNull(ls_RoomCd) THEN 
	ls_RoomCd = '%'
ELSE
	ls_header += "   사용장소 : "+ls_RoomCd1+"  호실명: "+ls_room+"  "
END IF
...
//       tab_1.tabpage_2.dw_print.Object.t_systime.Text = String(ldt_SysDateTime,'HH:MM:SS')
       ...
       IF ll_Rowcount = 0 THEN
		 ...
		 ELSE
			 //wf_setmenu('P',TRUE)
	       wf_SetMsg('자료가 조회되었습니다.')
			 tab_1.tabpage_2.dw_print.object.t_3.text = ls_header
		 END IF
END CHOOSE
return 1
//////////////////////////////////////////////////////////////////////////////////////////
...
avc_data.SetProperty('title', "재물조사표")
...
type ln_templeft from w_msheet`ln_templeft within w_hst302i
end type

type ln_tempright from w_msheet`ln_tempright within w_hst302i
end type

type ln_temptop from w_msheet`ln_temptop within w_hst302i
end type

type ln_tempbuttom from w_msheet`ln_tempbuttom within w_hst302i
end type

type ln_tempbutton from w_msheet`ln_tempbutton within w_hst302i
end type

type ln_tempstart from w_msheet`ln_tempstart within w_hst302i
end type

type uc_retrieve from w_msheet`uc_retrieve within w_hst302i
end type

type uc_insert from w_msheet`uc_insert within w_hst302i
end type

type uc_delete from w_msheet`uc_delete within w_hst302i
end type

type uc_save from w_msheet`uc_save within w_hst302i
end type

type uc_excel from w_msheet`uc_excel within w_hst302i
end type

type uc_print from w_msheet`uc_print within w_hst302i
end type

type st_line1 from w_msheet`st_line1 within w_hst302i
end type

type st_line2 from w_msheet`st_line2 within w_hst302i
end type

type st_line3 from w_msheet`st_line3 within w_hst302i
end type

type uc_excelroad from w_msheet`uc_excelroad within w_hst302i
end type

type ln_dwcon from w_msheet`ln_dwcon within w_hst302i
end type
	...
	CASE 1
	  ...
	  IF ll_rowcount <> 0 THEN
	     ...
	     FOR idx = 1 TO ll_rowcount
            ...
            idw_name.object.is_inve[idx] = 2          // 변경으로 바꾼다 
	...
	CASE 1
	  ...
	  IF ll_rowcount <> 0 THEN
	     ...
	     FOR idx = 1 TO ll_rowcount
            ...
            idw_name.object.is_inve[idx] = 0         // 변경으로 바꾼다 
