$PBExportHeader$w_hyk206a.srw
$PBExportComments$[w_list] 기타업적 평가자료 등록
forward
global type w_hyk206a from w_window
end type
type dw_main from uo_grid within w_hyk206a
end type
type dw_con from uo_dw within w_hyk206a
end type
type p_1 from picture within w_hyk206a
end type
type st_main from statictext within w_hyk206a
end type
type dw_print from datawindow within w_hyk206a
end type
end forward
...
lvc_data.setProperty('column1', 'inout_gb')  //교내외구분
lvc_data.setProperty('key1', 'coll_opt')
...
type ln_templeft from w_window`ln_templeft within w_hyk206a
end type

type ln_tempright from w_window`ln_tempright within w_hyk206a
end type

type ln_temptop from w_window`ln_temptop within w_hyk206a
end type

type ln_tempbuttom from w_window`ln_tempbuttom within w_hyk206a
end type

type ln_tempbutton from w_window`ln_tempbutton within w_hyk206a
end type

type ln_tempstart from w_window`ln_tempstart within w_hyk206a
end type

type uc_retrieve from w_window`uc_retrieve within w_hyk206a
end type

type uc_insert from w_window`uc_insert within w_hyk206a
end type

type uc_delete from w_window`uc_delete within w_hyk206a
end type

type uc_save from w_window`uc_save within w_hyk206a
end type

type uc_excel from w_window`uc_excel within w_hyk206a
end type

type uc_print from w_window`uc_print within w_hyk206a
end type

type st_line1 from w_window`st_line1 within w_hyk206a
end type

type st_line2 from w_window`st_line2 within w_hyk206a
end type

type st_line3 from w_window`st_line3 within w_hyk206a
end type

type uc_excelroad from w_window`uc_excelroad within w_hyk206a
end type
...
event type long ue_retrieve();String		ls_evl_ym, ls_member_no
Long		ll_rv
...
ll_rv = THIS.Retrieve(ls_evl_ym, ls_member_no)

dw_print.Retrieve(ls_evl_ym, ls_member_no)
...
event ue_insertend;call super::ue_insertend;String ls_evl_ym, ls_member_no, ls_mng_no, ls_evl_ym1
Long l_mng_no
...
dw_main.object.EVL_YM[al_row] = ls_EVL_YM
dw_main.object.MEMBER_NO[al_row] = ls_MEMBER_NO

dw_main.object.EVL_AREA[al_row] = '9'
...
boolean bringtotop = true
string title = "none"
string dataobject = "d_hyk206a_10"
