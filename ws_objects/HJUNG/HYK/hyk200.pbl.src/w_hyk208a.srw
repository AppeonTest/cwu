$PBExportHeader$w_hyk208a.srw
$PBExportComments$[w_list_list] 봉사업적평가자료 조정 및 확인
forward
global type w_hyk208a from w_window
end type
type dw_list from uo_grid within w_hyk208a
end type
type dw_con from uo_dw within w_hyk208a
end type
type dw_main from uo_grid within w_hyk208a
end type
type uc_row_insert from u_picture within w_hyk208a
end type
type uc_row_delete from u_picture within w_hyk208a
end type
type st_main from statictext within w_hyk208a
end type
type p_1 from picture within w_hyk208a
end type
type p_2 from picture within w_hyk208a
end type
type st_detail from statictext within w_hyk208a
end type
end forward
...
type ln_templeft from w_window`ln_templeft within w_hyk208a
end type

type ln_tempright from w_window`ln_tempright within w_hyk208a
end type

type ln_temptop from w_window`ln_temptop within w_hyk208a
end type

type ln_tempbuttom from w_window`ln_tempbuttom within w_hyk208a
end type

type ln_tempbutton from w_window`ln_tempbutton within w_hyk208a
end type

type ln_tempstart from w_window`ln_tempstart within w_hyk208a
end type

type uc_retrieve from w_window`uc_retrieve within w_hyk208a
end type

type uc_insert from w_window`uc_insert within w_hyk208a
end type

type uc_delete from w_window`uc_delete within w_hyk208a
end type

type uc_save from w_window`uc_save within w_hyk208a
end type

type uc_excel from w_window`uc_excel within w_hyk208a
end type

type uc_print from w_window`uc_print within w_hyk208a
end type

type st_line1 from w_window`st_line1 within w_hyk208a
end type

type st_line2 from w_window`st_line2 within w_hyk208a
end type

type st_line3 from w_window`st_line3 within w_hyk208a
end type

type uc_excelroad from w_window`uc_excelroad within w_hyk208a
end type
...
dw_main.reset()
If ls_member_no = '' Or isnull(ls_member_no)  then ls_member_no = '%'
...
Choose Case dwo.name
	Case 'evl_item'
		...
		this.object.point[row] = ll_point
		this.object.evl_area[row] = '3'
End Choose
...
String		ls_ym, ls_member_no, ls_gb, ls_mng_no
Long		ll_rv

ll_row = dw_list.GetRow()
If ll_row <= 0 Then RETURN -1

ls_ym = dw_list.object.evl_ym[ll_row]
ls_member_no = dw_list.object.member_no[ll_row]
ls_gb = dw_list.object.service_gb[ll_row]
ls_mng_no = dw_list.object.mng_no[ll_row]

ll_rv = dw_main.Retrieve(ls_ym, ls_member_no, ls_gb, ls_mng_no)
