$PBExportHeader$w_sch302a.srw
$PBExportComments$외박신청내역등록
forward
global type w_sch302a from w_window
end type
type dw_list from uo_grid within w_sch302a
end type
type dw_con from uo_dw within w_sch302a
end type
type dw_main from uo_dw within w_sch302a
end type
type p_1 from picture within w_sch302a
end type
type st_main from statictext within w_sch302a
end type
type p_2 from picture within w_sch302a
end type
type st_detail from statictext within w_sch302a
end type
end forward
...
lvc_data.setProperty('column1', 'reason_gb')  //외박사유
lvc_data.setProperty('key1', 'SAZ32')
...
Else
	...
	f_set_message("[확인] " + '외박신청을 할 수 있는 사생이 아닙니다. 기존자료가 존재한다면 조회만 가능합니다', '', parentwin)	
End If
...
type ln_templeft from w_window`ln_templeft within w_sch302a
end type

type ln_tempright from w_window`ln_tempright within w_sch302a
end type

type ln_temptop from w_window`ln_temptop within w_sch302a
end type

type ln_tempbuttom from w_window`ln_tempbuttom within w_sch302a
end type

type ln_tempbutton from w_window`ln_tempbutton within w_sch302a
end type

type ln_tempstart from w_window`ln_tempstart within w_sch302a
end type

type uc_retrieve from w_window`uc_retrieve within w_sch302a
end type

type uc_insert from w_window`uc_insert within w_sch302a
end type

type uc_delete from w_window`uc_delete within w_sch302a
end type

type uc_save from w_window`uc_save within w_sch302a
end type

type uc_excel from w_window`uc_excel within w_sch302a
end type

type uc_print from w_window`uc_print within w_sch302a
end type

type st_line1 from w_window`st_line1 within w_sch302a
end type

type st_line2 from w_window`st_line2 within w_sch302a
end type

type st_line3 from w_window`st_line3 within w_sch302a
end type

type uc_excelroad from w_window`uc_excelroad within w_sch302a
end type
