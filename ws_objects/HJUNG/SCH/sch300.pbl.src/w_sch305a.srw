$PBExportHeader$w_sch305a.srw
$PBExportComments$사생각종신청처리내역등록
forward
global type w_sch305a from w_window
end type
type dw_main from uo_grid within w_sch305a
end type
type dw_con from uo_dw within w_sch305a
end type
type p_1 from picture within w_sch305a
end type
type st_main from statictext within w_sch305a
end type
type uo_sms from uo_imgbtn within w_sch305a
end type
end forward
...
type ln_templeft from w_window`ln_templeft within w_sch305a
end type

type ln_tempright from w_window`ln_tempright within w_sch305a
end type

type ln_temptop from w_window`ln_temptop within w_sch305a
end type

type ln_tempbuttom from w_window`ln_tempbuttom within w_sch305a
end type

type ln_tempbutton from w_window`ln_tempbutton within w_sch305a
end type

type ln_tempstart from w_window`ln_tempstart within w_sch305a
end type

type uc_retrieve from w_window`uc_retrieve within w_sch305a
end type

type uc_insert from w_window`uc_insert within w_sch305a
end type

type uc_delete from w_window`uc_delete within w_sch305a
end type

type uc_save from w_window`uc_save within w_sch305a
end type

type uc_excel from w_window`uc_excel within w_sch305a
end type

type uc_print from w_window`uc_print within w_sch305a
end type

type st_line1 from w_window`st_line1 within w_sch305a
end type

type st_line2 from w_window`st_line2 within w_sch305a
end type

type st_line3 from w_window`st_line3 within w_sch305a
end type

type uc_excelroad from w_window`uc_excelroad within w_sch305a
end type
...
event type long ue_retrieve();String		ls_house_gb, ls_from_date, ls_to_date, ls_process_gb, ls_process_ty
Long		li_row, ll_rv
...
ls_process_ty = dw_con.object.process_ty[li_row]
...
event itemchanged;call super::itemchanged;Choose Case dwo.Name
	Case 'process_ty'
		Choose Case Data
			Case '1'
				dw_main.DataObject = 'd_sch305a_2'
				dw_main.Event Constructor()
			Case '2'
				dw_main.DataObject = 'd_sch305a_3'
				dw_main.Event Constructor()
			Case '3'
				dw_main.DataObject = 'd_sch305a_4'
				dw_main.Event Constructor()
		End Choose
End Choose
end event
...
ls_ty = '000' + trim(dw_con.object.process_ty[dw_con.getrow()])
