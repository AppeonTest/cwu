$PBExportHeader$w_sch301a.srw
$PBExportComments$상담신청내역등록
forward
global type w_sch301a from w_window
end type
type dw_list from uo_grid within w_sch301a
end type
type dw_con from uo_dw within w_sch301a
end type
type dw_main from uo_dw within w_sch301a
end type
type p_1 from picture within w_sch301a
end type
type st_main from statictext within w_sch301a
end type
type p_2 from picture within w_sch301a
end type
type st_detail from statictext within w_sch301a
end type
end forward
...
Else
	...
	f_set_message("[확인] " + '상담신청을 할 수 있는 사생이 아닙니다. 기존자료가 존재한다면 조회만 가능합니다', '', parentwin)	
End If


String ls_today, ls_fr_dt, ls_to_dt
ls_today = func.of_get_sdate('YYYYMMDD') 

SELECT ETC_CD1, ETC_CD2 
INTO :ls_fr_dt, :ls_to_dt
FROM CDDB.KCH102D 
WHERE UPPER(CODE_GB) = 'SUM30' 
AND  CODE = '1'
USING SQLCA;

If ls_fr_dt <> '' AND isnull(ls_fr_dt)= False then
	If ls_fr_dt > ls_today   or ls_today > ls_to_dt Then
...
type ln_templeft from w_window`ln_templeft within w_sch301a
end type

type ln_tempright from w_window`ln_tempright within w_sch301a
end type

type ln_temptop from w_window`ln_temptop within w_sch301a
end type

type ln_tempbuttom from w_window`ln_tempbuttom within w_sch301a
end type

type ln_tempbutton from w_window`ln_tempbutton within w_sch301a
end type

type ln_tempstart from w_window`ln_tempstart within w_sch301a
end type

type uc_retrieve from w_window`uc_retrieve within w_sch301a
integer x = 2391
integer width = 265
boolean originalsize = true
end type

type uc_insert from w_window`uc_insert within w_sch301a
integer x = 2670
integer width = 265
boolean originalsize = true
end type

type uc_delete from w_window`uc_delete within w_sch301a
integer x = 2949
integer width = 265
boolean originalsize = true
end type

type uc_save from w_window`uc_save within w_sch301a
integer x = 3232
integer width = 265
boolean originalsize = true
end type

type uc_excel from w_window`uc_excel within w_sch301a
integer x = 3511
integer width = 265
boolean originalsize = true
end type

type uc_print from w_window`uc_print within w_sch301a
integer width = 265
boolean originalsize = true
end type

type st_line1 from w_window`st_line1 within w_sch301a
end type

type st_line2 from w_window`st_line2 within w_sch301a
end type

type st_line3 from w_window`st_line3 within w_sch301a
end type

type uc_excelroad from w_window`uc_excelroad within w_sch301a
integer x = 3790
integer width = 366
boolean originalsize = true
end type
...
This.Object.std_year[al_row] 		= Trim(is_data[2])
This.Object.allocate_no[al_row] 	= Trim(is_data[3])
