$PBExportHeader$w_sch212a.srw
$PBExportComments$[w_print]  사생검색
forward
global type w_sch212a from w_window
end type
type dw_main from uo_grid within w_sch212a
end type
type dw_con from uo_dw within w_sch212a
end type
type p_1 from picture within w_sch212a
end type
type st_main from statictext within w_sch212a
end type
end forward
...
type ln_templeft from w_window`ln_templeft within w_sch212a
end type

type ln_tempright from w_window`ln_tempright within w_sch212a
end type

type ln_temptop from w_window`ln_temptop within w_sch212a
end type

type ln_tempbuttom from w_window`ln_tempbuttom within w_sch212a
end type

type ln_tempbutton from w_window`ln_tempbutton within w_sch212a
end type

type ln_tempstart from w_window`ln_tempstart within w_sch212a
end type

type uc_retrieve from w_window`uc_retrieve within w_sch212a
end type

type uc_insert from w_window`uc_insert within w_sch212a
end type

type uc_delete from w_window`uc_delete within w_sch212a
end type

type uc_save from w_window`uc_save within w_sch212a
end type

type uc_excel from w_window`uc_excel within w_sch212a
end type

type uc_print from w_window`uc_print within w_sch212a
end type

type st_line1 from w_window`st_line1 within w_sch212a
end type

type st_line2 from w_window`st_line2 within w_sch212a
end type

type st_line3 from w_window`st_line3 within w_sch212a
end type

type uc_excelroad from w_window`uc_excelroad within w_sch212a
end type
...
string     ls_house_gb, ls_std_year, ls_hakbun, ls_cell_no
...
ls_cell_no = '%' + func.of_nvl(dw_con.object.cell_no[dw_con.Getrow()], '%') + '%'
...
ll_rv = THIS.Retrieve(ls_house_gb, ls_std_year, ls_hakbun, ls_cell_no)
...
gvc_val.Setproperty('w_sch208p_hakbun', This.object.hakbun[row])
gvc_val.Setproperty('w_sch208p_kname', This.object.hname[row])
gvc_val.Setproperty('w_sch208p_house_gb', This.object.house_gb[row])
gvc_val.Setproperty('w_sch208p_std_year', This.object.std_year[row])


in_open.opensheetwithparm('w_sch208p', '', w_pf_main)
...
integer taborder = 10
boolean bringtotop = true
string dataobject = "d_sch212a_con"
boolean border = false
borderstyle borderstyle = stylebox!
end type
