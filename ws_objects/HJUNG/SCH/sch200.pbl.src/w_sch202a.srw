$PBExportHeader$w_sch202a.srw
$PBExportComments$[w_master_detail]입사신청서등록-기숙사용
forward
global type w_sch202a from w_window
end type
type dw_con from uo_dw within w_sch202a
end type
type dw_sub from uo_grid within w_sch202a
end type
type uc_row_insert from u_picture within w_sch202a
end type
type uc_row_delete from u_picture within w_sch202a
end type
type p_1 from picture within w_sch202a
end type
type st_main from statictext within w_sch202a
end type
type p_2 from picture within w_sch202a
end type
type st_detail from statictext within w_sch202a
end type
type st_1 from statictext within w_sch202a
end type
type p_image from picture within w_sch202a
end type
type dw_main from uo_dw within w_sch202a
end type
end forward
...
If Sqlca.Sqlcode = 0 Then
	...
	lvc_data.SetProperty('hakyun', ls_data[21])
Else
...
event ue_saveend;call super::ue_saveend;/* 기숙사생의 기본정보중 아래내역에 대한 사항을 확인하여 수정된경우, 학적과 연동하여 처리한다.
	- 핸드폰
	- 주소
	- 이메일
*/
...
MessageBox("확인",ls_memo)

Return 1

end event

type ln_templeft from w_window`ln_templeft within w_sch202a
end type

type ln_tempright from w_window`ln_tempright within w_sch202a
end type

type ln_temptop from w_window`ln_temptop within w_sch202a
end type

type ln_tempbuttom from w_window`ln_tempbuttom within w_sch202a
end type

type ln_tempbutton from w_window`ln_tempbutton within w_sch202a
end type

type ln_tempstart from w_window`ln_tempstart within w_sch202a
end type

type uc_retrieve from w_window`uc_retrieve within w_sch202a
end type

type uc_insert from w_window`uc_insert within w_sch202a
end type

type uc_delete from w_window`uc_delete within w_sch202a
end type

type uc_save from w_window`uc_save within w_sch202a
end type

type uc_excel from w_window`uc_excel within w_sch202a
end type

type uc_print from w_window`uc_print within w_sch202a
end type

type st_line1 from w_window`st_line1 within w_sch202a
end type

type st_line2 from w_window`st_line2 within w_sch202a
end type

type st_line3 from w_window`st_line3 within w_sch202a
end type

type uc_excelroad from w_window`uc_excelroad within w_sch202a
end type
...
This.Object.hakyun[al_row]			= lvc_data.GetProperty('hakyun')
This.Object.close_yn[al_row]		= 'N'

Return 1
end event

event ue_insertrow;call super::ue_insertrow;Return AncestorReturnValue
end event
