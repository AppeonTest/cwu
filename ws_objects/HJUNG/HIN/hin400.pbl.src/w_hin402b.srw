$PBExportHeader$w_hin402b.srw
$PBExportComments$호봉승급자료생성
forward
global type w_hin402b from w_msheet
end type
type dw_list from cuo_dwwindow_one_hin within w_hin402b
end type
type dw_update from cuo_dwwindow_one_hin within w_hin402b
end type
type dw_con from uo_dwfree within w_hin402b
end type
type uo_member from cuo_insa_member within w_hin402b
end type
type cb_create from uo_imgbtn within w_hin402b
end type
end forward

global type w_hin402b from w_msheet
string title = "호봉사정자료생성"
...
String	ls_Date
String	ls_DateGb
ls_DateGb =   MID(dw_con.object.date[1],1,1)	//1. 04월01일//MID(ddlb_date.Text,1,1)	//1. 04월01일
ls_Date   = MID(dw_con.object.date[1],4)//MID(ddlb_date.Text,4)
IF LEN(ls_Year) = 0 OR isNull(ls_Year) THEN
...
END IF
/////////////////////////////////////////////////////////////////////////////////
// 1.2.1 기준일자에서 숫자만을 가지고 온다(예 : 4월 -> 0401)
/////////////////////////////////////////////////////////////////////////////////
Long		ll_idx
Long		ll_Len
String	ls_Tmp
...
NEXT
is_SalDate = ls_Year + String(Integer(ls_Rtn),'00') + '01'
////////////////////////////////////////////////////////////////////////////////////
// 1.3 성명 입력여부 체크
////////////////////////////////////////////////////////////////////////////////////
...
ll_RowCnt = dw_update.Retrieve(is_JikJongCode,is_SalDate,is_KName)
...
DO WHILE ll_Row > 0
	...
	/////////////////////////////////////////////////////////////////////////////////
	li_ChagneOpt = 23	//변동구분(승급)
	DELETE	FROM	INDB.HIN007H
	WHERE		CHANGE_OPT = :li_ChagneOpt
	AND		FROM_DATE  = :is_SalDate
	AND		SAL_DATE   = :is_SalDate
...
type ln_templeft from w_msheet`ln_templeft within w_hin402b
end type

type ln_tempright from w_msheet`ln_tempright within w_hin402b
end type

type ln_temptop from w_msheet`ln_temptop within w_hin402b
end type

type ln_tempbuttom from w_msheet`ln_tempbuttom within w_hin402b
end type

type ln_tempbutton from w_msheet`ln_tempbutton within w_hin402b
end type

type ln_tempstart from w_msheet`ln_tempstart within w_hin402b
end type

type uc_retrieve from w_msheet`uc_retrieve within w_hin402b
end type

type uc_insert from w_msheet`uc_insert within w_hin402b
end type

type uc_delete from w_msheet`uc_delete within w_hin402b
end type

type uc_save from w_msheet`uc_save within w_hin402b
end type

type uc_excel from w_msheet`uc_excel within w_hin402b
end type

type uc_print from w_msheet`uc_print within w_hin402b
end type

type st_line1 from w_msheet`st_line1 within w_hin402b
end type

type st_line2 from w_msheet`st_line2 within w_hin402b
end type

type st_line3 from w_msheet`st_line3 within w_hin402b
end type

type uc_excelroad from w_msheet`uc_excelroad within w_hin402b
end type

type ln_dwcon from w_msheet`ln_dwcon within w_hin402b
end type
...
string dataobject = "d_hin402b_con"
boolean border = false
borderstyle borderstyle = stylebox!
end type
...
dw_list.Reset()
ll_RowCnt = dw_list.Retrieve(is_JikJongCode,is_SalDate,is_KName)
...
Integer		li_SalClassSeq			//호봉코드 뒤 두자리
...
FOR ll_Row = 1 TO ll_RowCnt
	...
	li_ChangeOpt        = 23													//변동코드(승급:23)
	ls_AnnounceCon      = '정기승급'											//변동내역
	ls_DeptCode         = dw_list.Object.dept_code        [ll_Row]	//부서코드
	...
	////////////////////////////////////////////////////////////////////////////////////
	// 3.1 다음 호봉코드, 호봉산정일, 호봉잔여월 처리
	////////////////////////////////////////////////////////////////////////////////////
	ls_NextSalYear      = dw_list.Object.com_next_sal_year[ll_Row]	//다음호봉년도
	ls_NextSalDate      = is_SalDate											//다음호봉일자
	li_NextSalLeftMonth = dw_list.Object.com_sal_leftmonth[ll_Row]	//다음호봉잔여월수
	li_SalClassSeq      = Integer(MID(ls_SalClass,2,2)) + 1			//호봉코드 뒤 두자리
	ls_NextSalClass     = MID(ls_SalClass,1,1) + String(li_SalClassSeq,'00')
	ls_JikJongCode      = MID(ls_DutyCode,1,1)
	IF ls_JikJongCode = '1' OR ls_JikJongCode = '2' THEN
		/////////////////////////////////////////////////////////////////////////////////
		// 3.1.1 직급이 교원, 조교인 경우처리
		//			호봉코드가 35을 넘으면 근가(G)호봉으로 처리
		/////////////////////////////////////////////////////////////////////////////////
		IF isNull(ls_NextSalClass) OR ls_NextSalClass = '000' THEN ls_NextSalClass = '001'
		IF ls_JikJongCode = '1' AND &
			ls_NextSalClass > '035' AND &
			MID(ls_NextSalClass,1,1) <> 'G' AND &
			MID(ls_NextSalClass,1,1) <> 'S' THEN
			ls_NextSalClass = 'G' + String(li_SalClassSeq - 35,'00')
		END IF
		IF ls_NextSalClass > 'G10' AND &
			MID(ls_NextSalClass,1,1) <> 'S' THEN ls_NextSalClass = 'G10'
	END IF
...
IF ll_InsRow > 0 THEN
//	wf_SetMenuBtn('SDR')
	wf_SetMsg('호봉사정 생성되었습니다. 확인 후 자료를 저장하시기 바랍니다.')
ELSE
//	wf_SetMenuBtn('R')
	wf_SetMsg('호봉사정대상자가 없습니다.')
END IF
