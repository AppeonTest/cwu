$PBExportHeader$w_hin805p.srw
$PBExportComments$전임교원 재임용 현황
forward
global type w_hin805p from w_msheet
end type
type st_3 from statictext within w_hin805p
end type
type dw_print from cuo_dwwindow_one_hin within w_hin805p
end type
type ddlb_date from dropdownlistbox within w_hin805p
end type
type em_year from editmask within w_hin805p
end type
type gb_1 from groupbox within w_hin805p
end type
end forward
...
//ls_day1 = mid(ls_day, 1, 4) + '.' + mid(ls_day, 5, 2) + '.' + mid(ls_day, 7, 2) + '.'
//dw_print.object.t_day1.text = ls_day1
