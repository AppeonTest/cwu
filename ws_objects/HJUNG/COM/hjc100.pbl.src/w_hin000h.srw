$PBExportHeader$w_hin000h.srw
$PBExportComments$인사기본정보도움말(성명,개인번호)
forward
global type w_hin000h from window
end type
type dw_main from uo_dwgrid within w_hin000h
end type
type ddlb_gubn from dropdownlistbox within w_hin000h
end type
type st_3 from statictext within w_hin000h
end type
type sle_member_no from singlelineedit within w_hin000h
end type
type st_8 from statictext within w_hin000h
end type
type sle_kname from singlelineedit within w_hin000h
end type
type st_1 from statictext within w_hin000h
end type
type gb_1 from groupbox within w_hin000h
end type
type cb_retrieve from u_picture within w_hin000h
end type
type cb_ok from u_picture within w_hin000h
end type
type cb_cancel from u_picture within w_hin000h
end type
end forward
...
ll_RowCnt = dw_main.Retrieve(ls_KName,ls_MemberNo,ls_JikJongcode)
...
ls_JikJongcode = lstr_com.ls_item[03]	//교직원구분

ddlb_gubn.SelectItem(Integer(ls_JikJongcode))
...
ll_GetRow = dw_main.getrow() //dw_main.GetSelectedRow(0)
IF ll_GetRow = 0 THEN
	MessageBox('확인','자료를 선택 후 사용하시기 바랍니다')
	RETURN
END IF

ls_KName          = dw_main.Object.kname          [ll_GetRow]
