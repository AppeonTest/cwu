$PBExportHeader$w_hge506b.srw
$PBExportComments$월 지급 강사료 생성(전체/학과별 생성)
forward
global type w_hge506b from w_tabsheet
end type
type tabpage_sheet02 from userobject within tab_sheet
end type
type tabpage_sheet02 from userobject within tab_sheet
end type
type hpb_1 from hprogressbar within w_hge506b
end type
type st_status from statictext within w_hge506b
end type
type cb_1 from commandbutton within w_hge506b
end type
type uo_yearhakgi from cuo_yyhakgi within w_hge506b
end type
type uo_month from cuo_month within w_hge506b
end type
type st_31 from statictext within w_hge506b
end type
type uo_gwa from cuo_hakgwa within w_hge506b
end type
type gb_4 from groupbox within w_hge506b
end type
type rb_all from radiobutton within w_hge506b
end type
type rb_gwa from radiobutton within w_hge506b
end type
type rb_member_no from radiobutton within w_hge506b
end type
type em_pay_date from editmask within w_hge506b
end type
type st_pay_date from statictext within w_hge506b
end type
type uo_insa from cuo_insa_member_gangsa within w_hge506b
end type
type dw_list002 from uo_dwgrid within w_hge506b
end type
type dw_list003 from uo_dwgrid within w_hge506b
end type
type pb_1 from uo_imgbtn within w_hge506b
end type
end forward
...
from		gedb.hpa111m a, gedb.hpa112t b
where		a.year		=		:is_yy
...
IF		is_hakgi	=	'1' OR is_hakgi = '2'	THEN
		...
		select	:is_yy, :is_hakgi, :ii_month, X.member_no, AA.pay_item_code,
					AA.SISU, 
					NVL(AA.AMT1,0),
		/*			+ NVL(AA.AMT2,0) + NVL(AA.AMT3,0) + NVL(AA.AMT4,0), */
					NVL(AA.HALF,0), 
		/*			NVL(AA.AMT1,0) + NVL(AA.AMT2,0) + NVL(AA.AMT3,0) + NVL(AA.AMT4,0) + NVL(AA.HALF,0),*/
					NVL(AA.AMT1,0) + NVL(AA.HALF,0),
					:is_pay_date, AA.holiday_opt,										
					:gstru_uid_uname.uid, :gstru_uid_uname.address, sysdate, 
					:gstru_uid_uname.uid, :gstru_uid_uname.address, sysdate
		from		GEDB.hpa111H X,
					(SELECT A.YEAR, A.MONTH, A.HAKGI, A.MEMBER_NO, SUM(B.PRICE_PER_TIME * C.NUM_OF_OVERTIME1) AMT1,
							 SUM(B.PRICE_PER_TIME * C.NUM_OF_OVERTIME2) AMT2,SUM(B.PRICE_PER_TIME * C.NUM_OF_OVERTIME3) AMT3,
...
/*							 + SUM(NVL(C.NUM_OF_OVERTIME2,0) )
																					+ SUM(NVL(C.NUM_OF_OVERTIME3,0) ) + SUM(NVL(C.NUM_OF_OVERTIME4,0)) SISU, */
							 SUM(NVL(C.NUM_OF_NIGENERAL,0)) HALF_SISU, A.holiday_opt
						 FROM   GEDB.HPA111H A, HAKSA.D_HPA102M B, GEDB.HPA112T C
						WHERE  A.YEAR	=	C.YEAR
						  AND  A.HAKGI	=	C.HAKGI
						  AND  A.MONTH	=	C.MONTH
...
ELSE		//계절학기일경우에는 + 3000원이다.
		...
		/*			NVL(AA.AMT1,0) + NVL(AA.AMT2,0) + NVL(AA.AMT3,0) + NVL(AA.AMT4,0) + NVL(AA.HALF,0),*/
					...
					:gstru_uid_uname.uid, :gstru_uid_uname.address, sysdate
		from		GEDB.hpa111H X,
					(SELECT A.YEAR, A.MONTH, A.HAKGI, A.MEMBER_NO, SUM((B.PRICE_PER_TIME + 3000) * C.NUM_OF_OVERTIME1) AMT1,
							 SUM((B.PRICE_PER_TIME + 3000) * C.NUM_OF_OVERTIME2) AMT2,SUM((B.PRICE_PER_TIME + 3000) * C.NUM_OF_OVERTIME3) AMT3,
...
//and		b.member_no	like	:is_member_no	
...
// 기초자료 Loop
//for	ll_cnt1 = 1 to ll_mast_count
//	ls_member_no			=	idw_mast.getitemstring(ll_cnt1, 'member_no')
//	li_num_of_time			=	idw_mast.getitemnumber(ll_cnt1, 'num_of_time')
//	li_num_of_general		=	idw_mast.getitemnumber(ll_cnt1, 'num_of_general')
//	li_num_of_middle		=	idw_mast.getitemnumber(ll_cnt1, 'num_of_middle')
//	li_num_of_large		=	idw_mast.getitemnumber(ll_cnt1, 'num_of_large')
//	li_num_of_nigeneral	=	idw_mast.getitemnumber(ll_cnt1, 'num_of_nigeneral')
//	li_num_of_nimiddle	=	idw_mast.getitemnumber(ll_cnt1, 'num_of_nimiddle')
//	li_num_of_nilarge		=	idw_mast.getitemnumber(ll_cnt1, 'num_of_nilarge')
//	li_limit_time			=	idw_mast.getitemnumber(ll_cnt1, 'limit_time')
//	
//	// 주별시수에 월을 Update한다.
//	update	padb.hpa112t
//	set		month	=	:ii_month
//	where		year				=	:is_yy
//	and		hakgi				=	:is_hakgi
//	and		week_weekend	>=	:li_str_weekend
//	and		week_weekend	<=	:li_end_weekend
...
//		
//	// 주차 Loop
//	for	ll_cnt2 = li_str_weekend to li_end_weekend
//		hpb_1.position += 1
//	next
//next
//MESSAGEBOX('','CREATE_1')
...
SetPointer(Arrow!)
//st_status.text = string(ll_mast_count) + '건의 자료가 생성되었습니다.!'
...
type ln_templeft from w_tabsheet`ln_templeft within w_hge506b
end type

type ln_tempright from w_tabsheet`ln_tempright within w_hge506b
end type

type ln_temptop from w_tabsheet`ln_temptop within w_hge506b
end type

type ln_tempbuttom from w_tabsheet`ln_tempbuttom within w_hge506b
end type

type ln_tempbutton from w_tabsheet`ln_tempbutton within w_hge506b
end type

type ln_tempstart from w_tabsheet`ln_tempstart within w_hge506b
end type

type uc_retrieve from w_tabsheet`uc_retrieve within w_hge506b
end type

type uc_insert from w_tabsheet`uc_insert within w_hge506b
end type

type uc_delete from w_tabsheet`uc_delete within w_hge506b
end type

type uc_save from w_tabsheet`uc_save within w_hge506b
end type

type uc_excel from w_tabsheet`uc_excel within w_hge506b
end type

type uc_print from w_tabsheet`uc_print within w_hge506b
end type

type st_line1 from w_tabsheet`st_line1 within w_hge506b
end type

type st_line2 from w_tabsheet`st_line2 within w_hge506b
end type

type st_line3 from w_tabsheet`st_line3 within w_hge506b
end type

type uc_excelroad from w_tabsheet`uc_excelroad within w_hge506b
end type

type ln_dwcon from w_tabsheet`ln_dwcon within w_hge506b
end type

type tab_sheet from w_tabsheet`tab_sheet within w_hge506b
...
type uo_tab from w_tabsheet`uo_tab within w_hge506b
boolean visible = false
integer x = 2501
integer y = 1552
end type

type dw_con from w_tabsheet`dw_con within w_hge506b
boolean visible = false
integer y = 276
integer width = 46
end type

type st_con from w_tabsheet`st_con within w_hge506b
integer y = 168
integer width = 4384
