$PBExportHeader$w_hpa505a.srw
$PBExportComments$주별 지급 시수 관리/출력
forward
global type w_hpa505a from w_tabsheet
end type
...
type uo_yearhakgi from cuo_yyhakgi within w_hpa505a
end type
type uo_2 from cuo_month within w_hpa505a
end type
type uo_str_week from cuo_week within w_hpa505a
end type
type uo_end_week from cuo_week within w_hpa505a
end type
type st_3 from statictext within w_hpa505a
end type
type rb_1 from radiobutton within w_hpa505a
end type
type rb_2 from radiobutton within w_hpa505a
end type
type rb_3 from radiobutton within w_hpa505a
end type
type uo_hakgwa from cuo_dept within w_hpa505a
end type
type rb_14 from radiobutton within w_hpa505a
end type
type rb_7 from radiobutton within w_hpa505a
end type
end forward
...
wf_setmsg('공통코드[인사기본정보_강사료구분]를 초기화 중입니다...')
idw_update[1].GetChild('sec_code',ldwc_Temp)
...
type ln_templeft from w_tabsheet`ln_templeft within w_hpa505a
end type

type ln_tempright from w_tabsheet`ln_tempright within w_hpa505a
end type

type ln_temptop from w_tabsheet`ln_temptop within w_hpa505a
end type

type ln_tempbuttom from w_tabsheet`ln_tempbuttom within w_hpa505a
end type

type ln_tempbutton from w_tabsheet`ln_tempbutton within w_hpa505a
end type

type ln_tempstart from w_tabsheet`ln_tempstart within w_hpa505a
end type

type uc_retrieve from w_tabsheet`uc_retrieve within w_hpa505a
end type

type uc_insert from w_tabsheet`uc_insert within w_hpa505a
end type

type uc_delete from w_tabsheet`uc_delete within w_hpa505a
end type

type uc_save from w_tabsheet`uc_save within w_hpa505a
end type

type uc_excel from w_tabsheet`uc_excel within w_hpa505a
end type

type uc_print from w_tabsheet`uc_print within w_hpa505a
end type

type st_line1 from w_tabsheet`st_line1 within w_hpa505a
...
type st_line2 from w_tabsheet`st_line2 within w_hpa505a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type st_line3 from w_tabsheet`st_line3 within w_hpa505a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type uc_excelroad from w_tabsheet`uc_excelroad within w_hpa505a
end type

type ln_dwcon from w_tabsheet`ln_dwcon within w_hpa505a
end type

type tab_sheet from w_tabsheet`tab_sheet within w_hpa505a
integer y = 328
integer width = 4384
integer height = 1968
integer taborder = 60
...
type uo_tab from w_tabsheet`uo_tab within w_hpa505a
end type

type dw_con from w_tabsheet`dw_con within w_hpa505a
boolean visible = false
end type

type st_con from w_tabsheet`st_con within w_hpa505a
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
//accepttext()
if dwo.name = 'retro_amt'  then
	setitem(row, 'pay_amt',	dec(data) + getitemnumber(row, 'month_amt') + getitemnumber(row, 'saneob'))

elseif dwo.name = 'month_amt' then
	setitem(row, 'pay_amt',	dec(data) + getitemnumber(row, 'saneob'))

elseif dwo.name = 'saneob' then
	setitem(row, 'pay_amt',	dec(data) + getitemnumber(row, 'month_amt'))
end if
