$PBExportHeader$w_hge507p.srw
$PBExportComments$월 강사료 지급 현황/명세서 출력
forward
global type w_hge507p from w_tabsheet
end type
...
type uo_yearhakgi from cuo_yyhakgi within w_hge507p
end type
type rb_prof from radiobutton within w_hge507p
end type
type rb_gang from radiobutton within w_hge507p
end type
type uo_month from cuo_month within w_hge507p
end type
type rb_4 from radiobutton within w_hge507p
end type
type rb_5 from radiobutton within w_hge507p
end type
type rb_lect from radiobutton within w_hge507p
end type
end forward
...
type ln_templeft from w_tabsheet`ln_templeft within w_hge507p
end type

type ln_tempright from w_tabsheet`ln_tempright within w_hge507p
end type

type ln_temptop from w_tabsheet`ln_temptop within w_hge507p
end type

type ln_tempbuttom from w_tabsheet`ln_tempbuttom within w_hge507p
end type

type ln_tempbutton from w_tabsheet`ln_tempbutton within w_hge507p
end type

type ln_tempstart from w_tabsheet`ln_tempstart within w_hge507p
end type

type uc_retrieve from w_tabsheet`uc_retrieve within w_hge507p
end type

type uc_insert from w_tabsheet`uc_insert within w_hge507p
end type

type uc_delete from w_tabsheet`uc_delete within w_hge507p
end type

type uc_save from w_tabsheet`uc_save within w_hge507p
end type

type uc_excel from w_tabsheet`uc_excel within w_hge507p
end type

type uc_print from w_tabsheet`uc_print within w_hge507p
end type

type st_line1 from w_tabsheet`st_line1 within w_hge507p
end type

type st_line2 from w_tabsheet`st_line2 within w_hge507p
end type

type st_line3 from w_tabsheet`st_line3 within w_hge507p
end type

type uc_excelroad from w_tabsheet`uc_excelroad within w_hge507p
end type

type ln_dwcon from w_tabsheet`ln_dwcon within w_hge507p
end type

type tab_sheet from w_tabsheet`tab_sheet within w_hge507p
integer x = 69
integer y = 328
integer width = 4384
integer textsize = -9
...
type uo_tab from w_tabsheet`uo_tab within w_hge507p
end type

type dw_con from w_tabsheet`dw_con within w_hge507p
boolean visible = false
end type

type st_con from w_tabsheet`st_con within w_hge507p
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
ELSEIF rb_lect.checked THEN
	this.object.t_head.Text = '강의전담교수'
ELSE
...
event clicked;   is_duty_code ='10'
	...
	idw_print.dataobject ='d_hge507p_3'
	idw_print.settransobject(sqlca)
	idw_print.Modify("DataWindow.Print.Preview='yes'")
	idw_print2.dataobject ='d_hge507p_2'
...
idw_print.dataobject ='d_hge507p_1'
idw_print.settransobject(sqlca)
idw_print.Modify("DataWindow.Print.Preview='yes'")
idw_print2.dataobject ='d_hge507p_4'
idw_print2.settransobject(sqlca)
idw_print2.Modify("DataWindow.Print.Preview='yes'")
//wf_getchild2()
wf_getchild()
...
event clicked;is_duty_code ='115'
ii_str_jikjong = 1
ii_end_jikjong = 1
