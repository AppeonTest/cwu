$PBExportHeader$cuo_member_fromto.sru
$PBExportComments$개인번호 From, To로 선택할 수 있다.
forward
global type cuo_member_fromto from userobject
end type
type dw_end_member from datawindow within cuo_member_fromto
end type
type dw_str_member from datawindow within cuo_member_fromto
end type
type st_6 from statictext within cuo_member_fromto
end type
type pb_str_find from picturebutton within cuo_member_fromto
end type
type pb_end_find from picturebutton within cuo_member_fromto
end type
end forward
...
global cuo_member_fromto cuo_member_fromto
...
public subroutine uf_getchild (integer ai_str_jikjong, integer ai_end_jikjong, string as_dept_code, integer ai_jaejik_opt)
...
if ai_jaejik_opt = 1 then
	uidw_str_child.setfilter("hin001m_jaejik_opt in (1, 2, 4)")
	uidw_str_child.filter()
elseif ai_jaejik_opt = 3 then
	uidw_str_child.setfilter("hin001m_jaejik_opt not in (1, 2, 4)")
	uidw_str_child.filter()
end if
...
//                   3=퇴직, 정년퇴직
if ai_jaejik_opt = 1 then
	uidw_end_child.setfilter("hin001m_jaejik_opt in (1, 2, 4)")
	uidw_end_child.filter()
elseif ai_jaejik_opt = 3 then
	uidw_end_child.setfilter("hin001m_jaejik_opt not in (1, 2, 4)")
	uidw_end_child.filter()
end if
