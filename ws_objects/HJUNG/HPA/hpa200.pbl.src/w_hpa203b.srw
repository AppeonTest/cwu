$PBExportHeader$w_hpa203b.srw
$PBExportComments$교원공제 생성/관리
forward
global type w_hpa203b from w_tabsheet
end type
...
type uo_yearmonth from cuo_yearmonth within w_hpa203b
end type
end forward

global type w_hpa203b from w_tabsheet
string title = "교원공제 생성/관리"
uo_yearmonth uo_yearmonth
end type
global w_hpa203b w_hpa203b
...
// 급여구분
idw_data.getchild('result_bit', idw_child)
idw_child.settransobject(sqlca)
if idw_child.retrieve('result_bit', 0) < 1 then
	idw_child.reset()
	idw_child.insertrow(0)
end if

idw_preview.getchild('result_bit', idw_child)
idw_child.settransobject(sqlca)
if idw_child.retrieve('result_bit', 0) < 1 then
	idw_child.reset()
	idw_child.insertrow(0)
...
idw_print.getchild('result_bit', idw_child)
idw_child.settransobject(sqlca)
if idw_child.retrieve('result_bit', 0) < 1 then
	idw_child.reset()
	idw_child.insertrow(0)
...
integer	li_find, i, li_column, li_start, li_col_len[] = {10, 13, 2, 7, 8, 3, 8, 6}
...
DO UNTIL ll_FileRead = -100
	...
	// 구분(2) = 개인별 자료		//표시코드 숫자로된 1자리(1:시작표시,2:데이타표시,3:집계표시)
	if left(ls_fileread, 1) <> '2' then	
		ll_FileRead = FileRead(li_FileOpen, ls_FileRead)
		continue
	end if
	ll_count ++
...
ll_FileRead = FileRead(li_FileOpen, ls_FileRead)
ll_row		= 0
li_column	= integer(idw_preview.object.datawindow.column.count) - 8		//저장할 대상이 8개임
...
DO UNTIL ll_FileRead = -100
	...
	and		jaejik_opt	in	(1, 2, 4)	//재직,퇴직예정,휴직
	and		member_no	in		
				(	select	member_no
...
type ln_templeft from w_tabsheet`ln_templeft within w_hpa203b
end type

type ln_tempright from w_tabsheet`ln_tempright within w_hpa203b
end type

type ln_temptop from w_tabsheet`ln_temptop within w_hpa203b
end type

type ln_tempbuttom from w_tabsheet`ln_tempbuttom within w_hpa203b
end type

type ln_tempbutton from w_tabsheet`ln_tempbutton within w_hpa203b
end type

type ln_tempstart from w_tabsheet`ln_tempstart within w_hpa203b
end type

type uc_retrieve from w_tabsheet`uc_retrieve within w_hpa203b
end type

type uc_insert from w_tabsheet`uc_insert within w_hpa203b
end type

type uc_delete from w_tabsheet`uc_delete within w_hpa203b
end type

type uc_save from w_tabsheet`uc_save within w_hpa203b
end type

type uc_excel from w_tabsheet`uc_excel within w_hpa203b
end type

type uc_print from w_tabsheet`uc_print within w_hpa203b
end type

type st_line1 from w_tabsheet`st_line1 within w_hpa203b
...
type st_line2 from w_tabsheet`st_line2 within w_hpa203b
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type st_line3 from w_tabsheet`st_line3 within w_hpa203b
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type uc_excelroad from w_tabsheet`uc_excelroad within w_hpa203b
end type

type ln_dwcon from w_tabsheet`ln_dwcon within w_hpa203b
end type

type tab_sheet from w_tabsheet`tab_sheet within w_hpa203b
integer y = 328
integer width = 4384
integer height = 1964
integer textsize = -9
...
type uo_tab from w_tabsheet`uo_tab within w_hpa203b
end type

type dw_con from w_tabsheet`dw_con within w_hpa203b
integer width = 0
integer height = 0
end type

type st_con from w_tabsheet`st_con within w_hpa203b
end type
