$PBExportHeader$w_hpa205b.srw
$PBExportComments$건강보험료정산 생성/관리
forward
global type w_hpa205b from w_tabsheet
end type
...
type uo_yearmonth from cuo_yearmonth within w_hpa205b
end type
end forward

global type w_hpa205b from w_tabsheet
string title = "건강보험료정산 생성/관리"
uo_yearmonth uo_yearmonth
end type
global w_hpa205b w_hpa205b
...
// Upload File Open
Integer	li_Rtn, li_err, li_cnt, li_jaejik_opt, li_jikjong_code, li_skip
String	ls_FullName, ls_retire_date
String	ls_FileName
String	ls_message
string	ls_kname
...
integer	li_find, i, li_column, li_start, li_col_len[] = {6, 8, 1, 2, 3, 11, 16, 13, 15, 15, 2, 15, 15, 15, 15, 15, 15, 15, 15}
...
//	select	nvl(a.member_no, '')
//	into		:ls_member_no
//	from		indb.hin001m a, indb.hin003m_02v b
//	where		a.jumin_no		=	:ls_jumin_no	
//	and		a.jaejik_opt	in	(1, 2, 4)
//	and		b.jikjong_code	<>	9
...
//					and		rownum	=	1	)	;
	...
	select	nvl(a.member_no, ' '), a.jaejik_opt, nvl(a.retire_date, ' '), b.jikjong_code
	into		:ls_member_no, :li_jaejik_opt, :ls_retire_date, :li_jikjong_code
	from		indb.hin001m a, indb.hin003m_02v b
	where		a.jumin_no		=	:ls_jumin_no	
	and		a.duty_code		=	b.duty_code
	and		a.member_no	in		
	...
	if sqlca.sqlcode <> 0 then
		...
		// 퇴직자는 Insert 하지 않는다.
		if (li_jaejik_opt <> 1 and li_jaejik_opt <> 2 and li_jaejik_opt <> 4) or (trim(ls_retire_date) <> '' and left(trim(ls_retire_date), 6) <> is_yearmonth) then
			f_messagebox('3', '[' + ls_kname + '][' + string(ls_jumin_no, '@@@@@@-@@@@@@@') + ']는 퇴직자 이므로 생성하지 않습니다.!')
			tab_sheet.tabpage_sheet01.hpb_1.position ++
			ll_FileRead = FileRead(li_FileOpen, ls_FileRead)
			li_skip	++
			continue
		// 기타직은 Insert 하지 않는다.
		elseif li_jikjong_code = 9 then
			f_messagebox('3', '[' + ls_kname + '][' + string(ls_jumin_no, '@@@@@@-@@@@@@@') + ']는 기타직 이므로 생성하지 않습니다.!')
			tab_sheet.tabpage_sheet01.hpb_1.position ++
			ll_FileRead = FileRead(li_FileOpen, ls_FileRead)
			li_skip	++
			continue
...
ll_rowcount	=	idw_preview.rowcount()

for	ll_row = ll_rowcount to 1	step -1
	if isnull(idw_preview.getitemstring(ll_row, 'year_month')) or trim(idw_preview.getitemstring(ll_row, 'year_month')) = '' then
		idw_preview.deleterow(ll_row)
	end if
next

if li_err > 0 then
	f_messagebox('1', string(li_cnt) + '건의 자료를 생성했습니다.!~n~n' + string(li_skip) + '건의 생성하지 않은 자료와~n' + string(li_err) + '건의 정확하지 않은 자료가 있습니다. 확인하시기 바랍니다.!')
else
...
type ln_templeft from w_tabsheet`ln_templeft within w_hpa205b
end type

type ln_tempright from w_tabsheet`ln_tempright within w_hpa205b
end type

type ln_temptop from w_tabsheet`ln_temptop within w_hpa205b
end type

type ln_tempbuttom from w_tabsheet`ln_tempbuttom within w_hpa205b
end type

type ln_tempbutton from w_tabsheet`ln_tempbutton within w_hpa205b
end type

type ln_tempstart from w_tabsheet`ln_tempstart within w_hpa205b
end type

type uc_retrieve from w_tabsheet`uc_retrieve within w_hpa205b
end type

type uc_insert from w_tabsheet`uc_insert within w_hpa205b
end type

type uc_delete from w_tabsheet`uc_delete within w_hpa205b
end type

type uc_save from w_tabsheet`uc_save within w_hpa205b
end type

type uc_excel from w_tabsheet`uc_excel within w_hpa205b
end type

type uc_print from w_tabsheet`uc_print within w_hpa205b
end type

type st_line1 from w_tabsheet`st_line1 within w_hpa205b
...
type st_line2 from w_tabsheet`st_line2 within w_hpa205b
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type st_line3 from w_tabsheet`st_line3 within w_hpa205b
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
...
type uc_excelroad from w_tabsheet`uc_excelroad within w_hpa205b
end type

type ln_dwcon from w_tabsheet`ln_dwcon within w_hpa205b
end type

type tab_sheet from w_tabsheet`tab_sheet within w_hpa205b
integer y = 320
integer width = 4384
integer height = 1960
integer textsize = -9
...
type uo_tab from w_tabsheet`uo_tab within w_hpa205b
integer x = 1463
integer y = 328
end type

type dw_con from w_tabsheet`dw_con within w_hpa205b
boolean visible = false
integer width = 0
integer height = 0
end type

type st_con from w_tabsheet`st_con within w_hpa205b
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Tahoma"
