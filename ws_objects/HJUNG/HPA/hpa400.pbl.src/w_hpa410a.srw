$PBExportHeader$w_hpa410a.srw
$PBExportComments$기부금관리
forward
global type w_hpa410a from w_tabsheet
end type
...
type uo_year from cuo_year within w_hpa410a
end type
type uo_member from cuo_insa_member within w_hpa410a
end type
type gb_1 from groupbox within w_hpa410a
end type
end forward
...
// 1.1 의료비 관계 DDDW초기화
////////////////////////////////////////////////////////////////////////////////////
f_set_message("[알림] " + '공통코드[기부금 관계구분]를 초기화 중입니다...', '', parentwin)
//f_dis_msg(1,'공통코드[기부금 관계구분]를 초기화 중입니다...','')
tab_sheet.tabpage_sheet01.dw_update.GetChild('gibo_code',ldwc_Temp)
ldwc_Temp.SetTransObject(SQLCA)
IF ldwc_Temp.Retrieve('gibo_opt',0) = 0 THEN
	wf_setmsg('공통코드[기부금 관계]를 입력하시기 바랍니다.')
	idw_child.InsertRow(0)
END IF

tab_sheet.tabpage_1.dw_print.GetChild('gibo_code',ldwc_Temp)
ldwc_Temp.SetTransObject(SQLCA)
IF ldwc_Temp.Retrieve('gibo_opt',0) = 0 THEN
	wf_setmsg('공통코드[기부금 관계]를 입력하시기 바랍니다.')
	idw_child.InsertRow(0)
...
DO WHILE ll_Row > 0
	...
	IF ldis_Status = New! OR ldis_Status = NewModified! THEN
		...
		// 3.1.1 기부금 항번 생성
		//////////////////////////////////////////////////////////////////////////////
		ls_MEMBER_NO = idw_name.Object.MEMBER_NO[ll_Row]	
		wf_SetMsg('기부금 항번 생성 중 입니다...')
		IF lb_Start THEN
			lb_Start = FALSE
...
//			messagebox('기부금 항번',ll_Seq_NO)
			...
			CHOOSE CASE SQLCA.SQLCODE
				...
				CASE 100
					ll_Seq_NO = 1
				CASE ELSE
					wf_SetMsg('기부금 항번 생성 중 오류가 발생하였습니다.')
					MessageBox('오류',&
									'기부금 항번 생성시 전산장애가 발생되었습니다.~r~n' + &
									'하단의 장애번호와 장애내역을~r~n' + &
...
type ln_templeft from w_tabsheet`ln_templeft within w_hpa410a
end type

type ln_tempright from w_tabsheet`ln_tempright within w_hpa410a
end type

type ln_temptop from w_tabsheet`ln_temptop within w_hpa410a
end type

type ln_tempbuttom from w_tabsheet`ln_tempbuttom within w_hpa410a
end type

type ln_tempbutton from w_tabsheet`ln_tempbutton within w_hpa410a
end type

type ln_tempstart from w_tabsheet`ln_tempstart within w_hpa410a
end type

type uc_retrieve from w_tabsheet`uc_retrieve within w_hpa410a
end type

type uc_insert from w_tabsheet`uc_insert within w_hpa410a
end type

type uc_delete from w_tabsheet`uc_delete within w_hpa410a
end type

type uc_save from w_tabsheet`uc_save within w_hpa410a
end type

type uc_excel from w_tabsheet`uc_excel within w_hpa410a
end type

type uc_print from w_tabsheet`uc_print within w_hpa410a
end type

type st_line1 from w_tabsheet`st_line1 within w_hpa410a
end type

type st_line2 from w_tabsheet`st_line2 within w_hpa410a
end type

type st_line3 from w_tabsheet`st_line3 within w_hpa410a
end type

type uc_excelroad from w_tabsheet`uc_excelroad within w_hpa410a
end type

type ln_dwcon from w_tabsheet`ln_dwcon within w_hpa410a
end type

type tab_sheet from w_tabsheet`tab_sheet within w_hpa410a
integer x = 18
integer y = 204
integer width = 3835
integer height = 2300
...
type uo_tab from w_tabsheet`uo_tab within w_hpa410a
end type

type dw_con from w_tabsheet`dw_con within w_hpa410a
end type

type st_con from w_tabsheet`st_con within w_hpa410a
end type
...
boolean titlebar = true
string title = "기부금 내역"
string dataobject = "d_hpa410a_11"
boolean hscrollbar = true
boolean vscrollbar = true
