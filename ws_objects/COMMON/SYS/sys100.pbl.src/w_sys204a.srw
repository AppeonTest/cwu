$PBExportHeader$w_sys204a.srw
$PBExportComments$전산업무알림조회
forward
global type w_sys204a from w_window
end type
type dw_list from uo_grid within w_sys204a
end type
type dw_con from uo_dw within w_sys204a
end type
type dw_main from uo_dw within w_sys204a
end type
type p_1 from picture within w_sys204a
end type
type st_main from statictext within w_sys204a
end type
type p_2 from picture within w_sys204a
end type
type st_detail from statictext within w_sys204a
end type
end forward
...
type ln_templeft from w_window`ln_templeft within w_sys204a
end type

type ln_tempright from w_window`ln_tempright within w_sys204a
end type

type ln_temptop from w_window`ln_temptop within w_sys204a
end type

type ln_tempbuttom from w_window`ln_tempbuttom within w_sys204a
end type

type ln_tempbutton from w_window`ln_tempbutton within w_sys204a
end type

type ln_tempstart from w_window`ln_tempstart within w_sys204a
end type

type uc_retrieve from w_window`uc_retrieve within w_sys204a
end type

type uc_insert from w_window`uc_insert within w_sys204a
end type

type uc_delete from w_window`uc_delete within w_sys204a
end type

type uc_save from w_window`uc_save within w_sys204a
end type

type uc_excel from w_window`uc_excel within w_sys204a
end type

type uc_print from w_window`uc_print within w_sys204a
end type

type st_line1 from w_window`st_line1 within w_sys204a
end type

type st_line2 from w_window`st_line2 within w_sys204a
end type

type st_line3 from w_window`st_line3 within w_sys204a
end type

type uc_excelroad from w_window`uc_excelroad within w_sys204a
end type
...
string dataobject = "d_sys204a_1"
boolean hscrollbar = true
boolean vscrollbar = true
boolean border = false
end type

event type long ue_retrieve();String		ls_area_gb, ls_from_dt, ls_to_dt
Long			ll_rv
...
ls_area_gb = func.of_nvl(dw_con.object.area_gb[dw_con.GetRow()], '%')
ls_from_dt = dw_con.object.from_dt[dw_con.GetRow()]
ls_to_dt = dw_con.object.to_dt[dw_con.GetRow()]

//If ls_area_gb = '' Or IsNull(ls_area_gb) Or ls_area_gb = 'null' Then ls_area_gb = '%'

dw_main.Reset()
ll_rv = This.Retrieve(ls_from_dt, ls_to_dt, '%')
...
integer taborder = 10
boolean bringtotop = true
string dataobject = "d_sys204a_con"
boolean border = false
...
String		ls_make_dt
Long		ll_rv, li_make_seq

ll_row = dw_list.GetRow()
If ll_row <= 0 Then RETURN -1

ls_make_dt = dw_list.object.make_dt[ll_row]
li_make_seq = dw_list.Object.req_seq[ll_row]

ll_rv = dw_main.Retrieve(ls_make_dt, li_make_seq)
